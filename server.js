...
// ⭐ Server-side authority for coin packages — never trust the client
const COIN_PACKAGES = {
...
// ⭐ Simple HTTPS JSON helper (used by NOWPayments)
...
// ⭐ Send gift — receiver gets ONLY target, NOT coins
app.post('/api/send-gift', async (req, res) => {
    try {
        ...
        // ❌ المستلم لا يحصل على عملات — فقط المرسل يخسر
        fromUser.coins -= amount;

        // ⭐ نصف قيمة الهدية يُضاف للتارجت فقط
...
app.post('/api/support/reply', async (req, res) => {
    ...
    if (userSocket) io.to(userSocket).emit('support-reply', { ticketId, reply });
...
app.post('/api/support/resolve', async (req, res) => {
    ...
    const userSocket = onlineUsers.get(ticket.username);
    if (userSocket) io.to(userSocket).emit('support-resolved', { ticketId });
...
// Sort object recursively (required for IPN signature verification)
...
// 1) Create a NOWPayments invoice and return the hosted invoice URL
app.post('/api/nowpayments/create-invoice', async (req, res) => {
    try {
        ...
        // Optionally pin a pay currency (e.g. usdttrc20)
...
// 2) IPN webhook — NOWPayments calls this on every status change
app.post('/api/nowpayments/ipn', async (req, res) => {
    try {
        ...
        // Credit only once, only when fully finished
