...
// ⭐ FIX: Notification schema now carries `type` + `data` so the client
//         can render the right UI and open the ticket modal on tap,
//         and so offline users can still receive the notification.
...
// ⭐ FIX: hardened admin check — always returns a proper boolean,
//         never throws on an empty/missing username.
...
// ⭐ FIX: notification now carries `type` + `data` (ticketId + full reply text)
//         so offline users can fetch and render it later.
app.post('/api/support/reply', async (req, res) => {
    try {
        ...
        // 1) Persist notification (works for offline users too)
        ...
        // 2) Live-push if the user is online
...
// ⭐ FIX: notification carries `type` + `data` so it survives offline users.
...
// ⭐ NEW: returns unread notifications for the user (used on login to catch up).
...
// ⭐ NEW: marks notifications as read.
