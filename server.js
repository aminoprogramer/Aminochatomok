...
const userSchema = new mongoose.Schema({
    ...
    monthlyLoginDays: { type: [Number], default: [] },   // ⭐ days of month (1-31)
...
// ⭐ NEW: streak = any N days in the current month (non-consecutive)
...
cron.schedule('0 0 1 * *', async () => {
    ...
    try {
        ...
        // ⭐ Users with target >= MIN_TARGET (streak already handled during login)
        ...
        for (const user of eligibleUsers) {
            const streak = user.monthlyLoginDays ? user.monthlyLoginDays.length : 0;
            if (streak < 10) continue;  // must have logged in at least 10 days
