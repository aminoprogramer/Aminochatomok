...
// ⭐⭐⭐ Socket.IO — محسّن للسرعة القصوى ⭐⭐⭐
const io = socketIo(server, {
    cors: { origin: "*", methods: ["GET", "POST"], credentials: true },
    maxHttpBufferSize: 5e7,
    pingTimeout: 20000,
    pingInterval: 10000,
    transports: ['websocket'],   // ⭐ WebSocket فقط = أسرع 2-3x
    allowEIO3: true,
    path: '/socket.io/',
    perMessageDeflate: {
        threshold: 1024,
...
app.use(express.static(__dirname, {
    maxAge: '7d',           // ⭐ cache للملفات الثابتة
    etag: true
}));

app.use((req, res, next) => {
    // ⭐ تسجيل فقط للأخطاء لتقليل I/O
...
mongoose.connect(MONGODB_URI, {
    maxPoolSize: 20,           // ⭐ pool اتصالات أكبر
    minPoolSize: 5,
    serverSelectionTimeoutMS: 5000,
    socketTimeoutMS: 45000,
    compressors: ['zlib'],     // ⭐ ضغط البيانات
    retryWrites: true
})
.then(async () => {
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (roomName) => {
        ...
        // ⭐ إرسال قائمة الغرف بشكل غير متزامن
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`⚡ Speed optimizations: WebSocket-only, HTTP compression, MongoDB indexing, in-memory cache`);
