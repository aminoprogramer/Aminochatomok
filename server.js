...
// ⭐ Escapes HTML so user-supplied strings can't break Telegram's parser.
function tgEscape(s) {
    return String(s ?? '').replace(/[&<>]/g, c => c === '&' ? '&amp;' : c === '<' ? '&lt;' : '&gt;');
}

// ⭐ Trims long strings so we never exceed Telegram's 4096-char limit.
...
//    Silently no-ops if env vars aren't set.
...
// ⭐ Convenience wrappers so call sites stay tidy.
...
async function updateLoginStreak(user) {
    ...
    if ((user.consecutiveLoginDays || 0) >= 10) {
        ...
        if (currentTarget >= MIN_TARGET) {
            ...
            // ⭐ Telegram: notify admin of payout created
...
app.post('/api/mark-payout-paid', async (req, res) => {
    ...
    // ⭐ Telegram: payout marked as paid
    tgNotifyPayoutPaid({
...
app.post('/api/manual-coin-request', uploadProof.single('proofFile'), async (req, res) => {
    ...
    // ⭐ Telegram: new manual coin request
...
app.post('/api/approve-manual-request', async (req, res) => {
    ...
    if (user) {
        ...
        // ⭐ Telegram: manual request approved
        sendTelegramNotification(
    ...
    try {
        ...
        if (FINAL_OK.includes(payment_status) && !payment.credited) {
            ...
            if (user) {
                ...
                // ⭐ Telegram: crypto purchase confirmed
...
cron.schedule('0 0 1 * *', async () => {
    ...
    try {
        ...
        for (const user of eligibleUsers) {
            ...
            // ⭐ Telegram: monthly reset payout created
            tgNotifyPayoutCreated({
                username: user.username,
                dollars,
                monthLabel,
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_ADMIN_CHAT_ID) {
        ...
        // Send a startup ping so you know it works
