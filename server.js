...
function calculatePayout(target) {
    return Math.floor((target || 0) / 100000) * 25;
}

// ⭐ Update login streak — payout requires 10 days AND target >= 100K
...
// ⭐ Send gift — adds half value to receiver's monthly target
app.post('/api/send-gift', async (req, res) => {
    try {
        ...
        const oldMilestone = Math.floor((toUser.monthlyTarget || 0) / 100000);
        ...
        const newMilestone = Math.floor(toUser.monthlyTarget / 100000);
...
app.get('/api/get-target-info', async (req, res) => {
    try {
        ...
        const nextMilestone = (Math.floor(monthlyTarget / 100000) + 1) * 100000;
...
// ========== CRON JOBS ==========
// ⭐ Monthly reset — Payout for qualifying users, reset for everyone
cron.schedule('0 0 1 * *', async () => {
    ...
    try {
        ...
        // 2) إشعار للذين لم يستوفوا الشروط
