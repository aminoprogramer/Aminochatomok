...
// ⭐ UPDATED: Support filtering by status (all / active / open / in-progress / resolved)
app.get('/api/support/tickets', async (req, res) => {
    try {
        ...
        if (status === 'all') {
            ...
            // Default: show only unresolved tickets
...
// ⭐ UPDATED: Send admin reply along with resolved event
app.post('/api/support/resolve', async (req, res) => {
    try {
        ...
        await Notification.create({
            ...
            message: `Ticket ${ticket.ticketId} was resolved${ticket.adminReply ? ': ' + ticket.adminReply.substring(0, 100) : ''}`,
            type: 'support_resolved',
            data: { ticketId: ticket.ticketId, reply: ticket.adminReply }
        });

        const userSocket = onlineUsers.get(normalizeUsername(ticket.username));
        if (userSocket) {
            // ⭐ Send the admin reply along with resolved event
