...
// ========== PROFILE PICTURE (UPDATED WITH VALIDATION) ==========
...
// ========== SECURITY QUESTION (WITH RATE LIMITING) ==========
...
app.get('/api/get-all-users', requireLogin, async (req, res) => {
  try {
    const users = await User.find({}, { username: 1, profile_pic: 1, isAdmin: 1, coins: 1, isVIP: 1 });
    res.json(users);
  } catch (err) {
    res.status(500).json([]);
  }
