// server.js
// ⭐⭐⭐ Mokalmat Chat — SECURE SERVER v3.1 + AUTO-BAN + ADMIN SYNC ⭐⭐⭐
...
setTelegramAlertSender((alert) => {
    ...
    if (alert.violations && alert.violations.length) {
        text += `\n📋 <b>Recent violations:</b>\n`;
        alert.violations.slice(0, 5).forEach(v => {
            text += `  • ${tgEscape(v.type)} (weight: ${v.weight})\n`;
        });
    }
...
.then(async () => {
    ...
    if (!adminExists) {
        ...
        // ⭐⭐⭐ جديد: تحديث كلمة المرور تلقائياً من ENV
        ...
        if (!passwordMatch) {
            ...
            console.log(`   New password length: ${adminPassword.length}`);
        } else {
            console.log(`✅ Admin exists with correct password from ENV`);
        }

        // ⭐ تحديث الأسئلة الأمنية أيضاً
...
function requireAuth(req, res, next) {
    ...
    if (!result.valid) {
        if (result.reason === 'fingerprint_mismatch') {
            handleViolation(ip, 'fingerprint_mismatch', result.payload?.username);
...
app.post('/api/register',
    ...
    async (req, res) => {
        try {
            ...
            if (!regCheck.allowed) {
                handleViolation(ip, 'register_flood');
                return res.status(429).json({
                    success: false,
...
app.post('/api/login',
    ...
    async (req, res) => {
        try {
            ...
            if (!ipCheck.allowed) {
                handleViolation(ip, 'login_flood', username);
                return res.status(429).json({
                    success: false,
            ...
            if (!userCheck.allowed) {
                handleViolation(ip, 'targeted_bruteforce', username);
                return res.status(429).json({
                    success: false,
            ...
            if (!ok) {
                ...
                if (user.failedLoginAttempts >= 5) {
                    ...
                    handleViolation(ip, 'bruteforce_lockout', username);
                }
                await user.save();
    ...
    async (req, res) => {
        try {
            ...
            if (!giftCheck.allowed) {
                handleViolation(req.clientIp, 'flood_gift', req.authUser);
...
app.post('/api/kick-user', requireAuth,
    ...
    async (req, res) => {
        ...
        if (!kickCheck.allowed) {
            handleViolation(req.clientIp, 'flood_kick', req.authUser);
...
app.post('/api/get-security-question',
    ...
    async (req, res) => {
        ...
        if (!check.allowed) {
            handleViolation(ip, 'login_flood');
            return res.status(429).json({ success: false });
        }
    ...
    async (req, res) => {
        try {
            ...
            if (!gameCheck.allowed) {
                handleViolation(req.clientIp, 'flood_game', req.authUser);
...
io.use(async (socket, next) => {
    ...
    if (!connCheck.allowed) {
        handleViolation(ip, 'connection_flood');
...
io.use(async (socket, next) => {
    try {
        ...
        if (!result.valid) {
            if (result.reason === 'fingerprint_mismatch') {
                handleViolation(socket.clientIp, 'socket_fp_mismatch', result.payload?.username);
            } else if (result.reason !== 'expired') {
                handleViolation(socket.clientIp, 'invalid_token', result.payload?.username);
            }
...
function isHighRisk(username) {
    return getSuspicionScore(username) >= 80;
}
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🛡️  Anti-Impersonation: fingerprint-bound sessions`);
    console.log(`🛡️  Anti-Flood: 5-layer rate limiting`);
    console.log(`🚫 AUTO-BAN: Immediate + Progressive + Chain banning`);
    console.log(`🔄 ADMIN SYNC: Password updates automatically from ENV`);
    if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_ADMIN_CHAT_ID) {
        console.log('📨 Telegram: ENABLED (with auto-ban alerts)');
        sendTelegramNotification('🚀 <b>Mokalmat server started</b>\n🔒 Secure mode + Auto-Ban active.');
    } else {
