...
app.post('/api/support/reply', async (req, res) => {
    try {
        ...
        // ⭐ Save notification in DB so user receives it even when offline
        try {
...
// ⭐ Resolve → DELETE the ticket + notify user (works online & offline)
app.post('/api/support/resolve', async (req, res) => {
    try {
        ...
        // ⭐ Save notification in DB so user gets it even if offline
        try {
            await Notification.create({
                username: normalizedUser,
...
// ⭐ Clear ALL tickets + notify every user (online & offline)
...
// ⭐ Fetch all unread notifications (called on login)
