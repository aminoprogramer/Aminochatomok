// ============================================================
//  FORCE IPv4 FOR ALL HTTPS/HTTP IN THIS PROCESS
//  Fixes node-telegram-bot-api ECONNABORTED on Node 20+
//  Must be at the VERY TOP, before anything else loads.
// ============================================================
const _https = require('https');
const _http = require('http');
const _OriginalHttpsAgent = _https.Agent;
const _OriginalHttpAgent = _http.Agent;

_https.Agent = function (opts) {
    return new _OriginalHttpsAgent(Object.assign({}, opts, { family: 4, keepAlive: true }));
};
_http.Agent = function (opts) {
    return new _OriginalHttpAgent(Object.assign({}, opts, { family: 4, keepAlive: true }));
};

_https.globalAgent = new _OriginalHttpsAgent({ family: 4, keepAlive: true });
_http.globalAgent = new _OriginalHttpAgent({ family: 4, keepAlive: true });

const _dns = require('dns');
if (_dns.setDefaultResultOrder) _dns.setDefaultResultOrder('ipv4first');
try { _dns.setServers(['8.8.8.8', '8.8.4.4']); } catch (_) {}

// Silence transient network noise from node-telegram-bot-api
...
async function updateLoginStreak(user) {
    ...
    if ((user.consecutiveLoginDays || 0) >= 10) {
    ...
    return {
        ...
        daysLeft: Math.max(0, 10 - (user.consecutiveLoginDays || 0)),
...
app.get('/api/get-target-info', async (req, res) => {
    try {
        ...
        const streak = user.consecutiveLoginDays || 0;
...
app.post('/api/support/reply', async (req, res) => {
    try {
        ...
        const userSocket = onlineUsers.get(normalizeUsername(ticket.username));
        if (userSocket) {
...
app.post('/api/support/resolve', async (req, res) => {
    try {
        ...
        const userSocket = onlineUsers.get(normalizeUsername(ticket.username));
        if (userSocket) {
            io.to(userSocket).emit('support-resolved', { ticketId: ticket.ticketId });
        }
    ...
    socket.on('user-online', async (username) => {
        ...
        if (u) {
            ...
            socket.emit('streak-status', {
                consecutiveLoginDays: u.consecutiveLoginDays || 0,
                daysLeft: Math.max(0, 10 - (u.consecutiveLoginDays || 0)),
...
cron.schedule('0 0 1 * *', async () => {
    ...
    try {
        ...
        const eligibleUsers = await User.find({
            monthlyTarget: { $gte: MIN_TARGET },
            consecutiveLoginDays: { $gte: MIN_STREAK }
        });
        ...
        const failedUsers = await User.find({
            $or: [
                { monthlyTarget: { $lt: MIN_TARGET, $gt: 0 } },
                { consecutiveLoginDays: { $lt: MIN_STREAK, $gt: 0 } }
            ]
        });
        for (const user of failedUsers) {
            ...
            if (sock) {
                let reason = '';
                if (user.monthlyTarget < MIN_TARGET && user.consecutiveLoginDays < MIN_STREAK) {
                    reason = `Target: ${(user.monthlyTarget||0).toLocaleString()}/${MIN_TARGET.toLocaleString()} · Streak: ${user.consecutiveLoginDays||0}/10`;
                } else if (user.monthlyTarget < MIN_TARGET) {
                    reason = `Target: ${(user.monthlyTarget||0).toLocaleString()}/${MIN_TARGET.toLocaleString()}`;
                } else {
                    reason = `Streak: ${user.consecutiveLoginDays||0}/10`;
                }
                io.to(sock).emit('monthly-reset', {
                    reason,
                    monthLabel,
                    lostTarget: user.monthlyTarget || 0,
                    lostStreak: user.consecutiveLoginDays || 0
                });
            }
        }

        const resetResult = await User.updateMany({}, {
            $set: { monthlyTarget: 0, consecutiveLoginDays: 0 }
        });
        console.log(`✅ Reset target & streak for ${resetResult.modifiedCount} users`);
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    try {
        ...
        startTelegramBot({
            ...
            app,
            models: {
                User, Room, Gift, FriendRequest, PrivateMessage,
                CoinRequest, BoostLog, ManualCoinRequest, PaymentTransaction,
                SupportTicket, Notification, Payout, CryptoPayment
            },
            helpers: {
                normalizeUsername,
                isAdmin,
