...
// ⭐ SECURITY: Trust proxy for correct client IP (set to true if behind nginx/cloudflare)
...
const AUTO_BAN_WINDOW_MS = (parseInt(process.env.AUTO_BAN_WINDOW_MIN) || 15) * 60 * 1000;
const AUTO_BAN_DURATION_MS = (parseInt(process.env.AUTO_BAN_DURATION_MIN) || 60) * 60 * 1000;
...
function tgNotifyIpBanned({ ip, reason, username, permanent, expiresAt }) {
    const expiry = permanent ? '♾️ Permanent' : `⏰ Until ${new Date(expiresAt).toLocaleString()}`;
    sendTelegramNotification(
        `🚫 <b>IP Banned</b>\n🌐 <code>${tgEscape(ip)}</code>\n👤 <code>${tgEscape(username || 'N/A')}</code>\n📝 ${tgEscape(reason)}\n${expiry}`
    );
}
function tgNotifyIpUnbanned({ ip, adminUsername }) {
    sendTelegramNotification(
        `✅ <b>IP Unbanned</b>\n🌐 <code>${tgEscape(ip)}</code>\n👑 <code>${tgEscape(adminUsername)}</code>`
    );
}
...
const ipBanSchema = new mongoose.Schema({
    ...
    bannedBy: { type: String, lowercase: true, required: true },
...
const ipWhitelistSchema = new mongoose.Schema({
    ip: { type: String, required: true, unique: true, index: true },
    addedBy: { type: String, lowercase: true, required: true },
...
ipLogSchema.index({ createdAt: 1 }, { expireAfterSeconds: 60 * 60 * 24 * 7 });
const IpLog = mongoose.model('IpLog', ipLogSchema);

// ============================================================
// ========== IP BAN RUNTIME STATE ============================
// ============================================================
const failedLoginMap = new Map();
const ipBanCache = new Map();
const IP_BAN_CACHE_TTL = 60 * 1000;
let lastBanCacheRefresh = 0;

async function refreshIpBanCache() {
    if (Date.now() - lastBanCacheRefresh < IP_BAN_CACHE_TTL) return;
    try {
        const now = new Date();
        ...
        lastBanCacheRefresh = Date.now();
    } catch (err) {
        console.error('refreshIpBanCache error:', err.message);
    }
}
...
async function checkIpBan(req, res, next) {
    ...
    try {
        const wl = await IpWhitelist.findOne({ ip }).lean();
        if (wl) return next();
    } catch (_) { }

    await refreshIpBanCache();

    const ban = ipBanCache.get(ip);
    if (ban) {
        if (!ban.permanent && ban.expiresAt && new Date(ban.expiresAt) <= new Date()) {
...
app.post('/api/login', authLimiter, async (req, res) => {
    try {
        ...
        const handleFailedLogin = async () => {
            const now = Date.now();
            let entry = failedLoginMap.get(ip);
            if (!entry || now - entry.firstAt > AUTO_BAN_WINDOW_MS) {
                entry = { count: 0, firstAt: now, lastAt: now };
            }
            entry.count++;
            entry.lastAt = now;
            ...
            if (entry.count >= AUTO_BAN_THRESHOLD) {
                const expiresAt = new Date(now + AUTO_BAN_DURATION_MS);
                await IpBan.updateOne(
                    { ip },
                    {
                        ip,
                        reason: `Auto-ban: ${entry.count} failed logins`,
                        bannedBy: 'system',
                        permanent: false,
                        expiresAt,
                        username: normalizeUsername(username),
                ...
                console.log(`🚫 Auto-banned IP ${ip} (${entry.count} failed logins)`);
                tgNotifyIpBanned({
                    ip,
                    reason: `Auto-ban: ${entry.count} failed logins`,
                    username: normalizeUsername(username),
...
app.get('/api/admin/ip-bans', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        if (status === 'active') {
            ...
            filter.permanent = false;
            filter.expiresAt = { $lte: new Date() };
        }

        const bans = await IpBan.find(filter).sort({ createdAt: -1 }).limit(500).lean();
...
app.post('/api/admin/ip-ban', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { ip, reason, duration, permanent } = req.body;
        if (!ip) return res.json({ success: false, error: 'IP required' });

        const cleanIp = normalizeIp(String(ip).trim());
        const isIPv4 = /^(\d{1,3}\.){3}\d{1,3}$/.test(cleanIp);
        const isIPv6 = /^[0-9a-f:]+$/i.test(cleanIp) && cleanIp.includes(':');
        if (!isIPv4 && !isIPv6) {
            return res.json({ success: false, error: 'Invalid IP format' });
        }

        if (cleanIp === req.clientIp) {
            return res.json({ success: false, error: 'Cannot ban your own IP' });
        }

        const durationMin = Math.max(1, Math.min(parseInt(duration) || 60, 60 * 24 * 365));
        const expiresAt = permanent ? null : new Date(Date.now() + durationMin * 60 * 1000);

        const ban = await IpBan.findOneAndUpdate(
            { ip: cleanIp },
            {
                ip: cleanIp,
                reason: sanitizeString(reason, 200) || 'Manual ban',
                bannedBy: req.authUsername,
                permanent: !!permanent,
                expiresAt,
                username: sanitizeString(req.body.username, 50) || null,
                userAgent: sanitizeString(req.body.userAgent, 200) || null
            },
            { upsert: true, new: true }
        );

        ipBanCache.set(cleanIp, {
            reason: ban.reason,
            expiresAt: ban.expiresAt,
            permanent: ban.permanent
        });
        ...
        for (const [username, socketId] of onlineUsers.entries()) {
            ...
            if (s && s.clientIp === cleanIp) {
                s.emit('banned', { reason: ban.reason });
                s.disconnect(true);
        ...
        console.log(`🚫 Admin ${req.authUsername} banned IP ${cleanIp}`);
...
app.post('/api/admin/ip-unban', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { ip } = req.body;
        if (!ip) return res.json({ success: false, error: 'IP required' });

        const cleanIp = normalizeIp(String(ip).trim());
        const result = await IpBan.deleteOne({ ip: cleanIp });
        ipBanCache.delete(cleanIp);
        failedLoginMap.delete(cleanIp);

        tgNotifyIpUnbanned({ ip: cleanIp, adminUsername: req.authUsername });
        console.log(`✅ Admin ${req.authUsername} unbanned IP ${cleanIp}`);
...
app.post('/api/admin/ip-whitelist', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        await IpWhitelist.findOneAndUpdate(
            { ip: cleanIp },
            { ip: cleanIp, addedBy: req.authUsername, note: sanitizeString(note, 200) },
            { upsert: true }
        );
        await IpBan.deleteOne({ ip: cleanIp });
        ipBanCache.delete(cleanIp);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.delete('/api/admin/ip-whitelist/:ip', requireAuth, requireAdmin, async (req, res) => {
    try {
        const cleanIp = normalizeIp(req.params.ip);
        await IpWhitelist.deleteOne({ ip: cleanIp });
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.get('/api/admin/ip-whitelist', requireAuth, requireAdmin, async (req, res) => {
    try {
        const list = await IpWhitelist.find().sort({ createdAt: -1 }).lean();
...
app.get('/api/admin/ip-logs', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { ip, username, limit } = req.query;
        const filter = {};
        if (ip) filter.ip = normalizeIp(ip);
        if (username) filter.username = normalizeUsername(username);
        const logs = await IpLog.find(filter)
            .sort({ createdAt: -1 })
...
app.get('/api/admin/ip-stats', requireAuth, requireAdmin, async (req, res) => {
    try {
        const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const [totalBans, activeBans, topFailed, recentBans] = await Promise.all([
            IpBan.countDocuments({}),
            IpBan.countDocuments({
                $or: [{ permanent: true }, { expiresAt: { $gt: new Date() } }]
            }),
            IpLog.aggregate([
                { $match: { action: 'login_fail', createdAt: { $gte: since } } },
                { $group: { _id: '$ip', count: { $sum: 1 }, lastSeen: { $max: '$createdAt' } } },
                { $sort: { count: -1 } },
                { $limit: 10 }
            ]),
            IpBan.find().sort({ createdAt: -1 }).limit(10).lean()
        ]);
        res.json({
            success: true,
            totalBans,
...
app.post('/api/nowpayments/ipn', async (req, res) => {
    try {
        ...
        if (!ipnSecret) {
            console.error('⚠️ IPN_SECRET not configured — rejecting');
...
io.on('connection', (socket) => {
    console.log('🔌 Socket connected:', socket.id, '→', socket.authUsername, 'from IP', socket.clientIp);
...
cron.schedule('*/5 * * * *', async () => {
    try {
        ...
        if (result.deletedCount > 0) {
            console.log(`🧹 Cleaned ${result.deletedCount} expired IP bans`);
            lastBanCacheRefresh = 0;
        }
        const cutoff = Date.now() - AUTO_BAN_WINDOW_MS * 2;
        for (const [ip, entry] of failedLoginMap) {
            if (entry.lastAt < cutoff) failedLoginMap.delete(ip);
        }
    } catch (err) {
        console.error('IP ban cleanup error:', err.message);
    }
});
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔐 SECURITY: bcrypt + rate-limit + helmet + socket-auth + IP-ban`);
    console.log(`🛡️  AUTO-BAN: ${AUTO_BAN_THRESHOLD} failed logins → ${AUTO_BAN_DURATION_MS / 60000}min ban`);
    if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_ADMIN_CHAT_ID) {
        console.log('📨 Telegram: ENABLED');
        sendTelegramNotification('🚀 <b>Mokalmat server started</b>\n🔐 Security + IP-ban active.');
    } else {
