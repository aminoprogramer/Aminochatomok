...
// ⭐ REMOVED: dns.setServers (breaks on many networks)
// If you really need it, wrap in try/catch — but generally unnecessary.
...
// ⭐⭐⭐ CRITICAL: trust proxy (for rate-limit + real IP behind reverse proxy)
...
const io = socketIo(server, {
    cors: {
        origin: process.env.CLIENT_ORIGIN || true,  // ⭐ reflect origin, not "*"
        methods: ["GET", "POST"],
        credentials: true
    },
    ...
    pingTimeout: 25000,
    pingInterval: 25000,
    transports: ['polling', 'websocket'],  // ⭐ POLLING FIRST (fallback for blocked WS)
    allowEIO3: true,
    path: '/socket.io/',
...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            defaultSrc: ["'self'"],
            scriptSrc: ["'self'", "https://cdn.socket.io"],  // ⭐ removed 'unsafe-inline'
            styleSrc: ["'self'", "'unsafe-inline'"],          // inline styles still used
...
// ⭐⭐⭐ BLOCK sensitive file leaks BEFORE static middleware
app.use((req, res, next) => {
    const p = req.path.toLowerCase();
    if (
        p.startsWith('/.env') ||
        p.startsWith('/server.js') ||
        p.startsWith('/package') ||
        p.startsWith('/.git') ||
        p.startsWith('/node_modules') ||
        p.startsWith('/sw.js.map') ||
        p.endsWith('.map')
    ) {
...
// ⭐ Add TLS for Atlas / remote clusters
...
app.post('/api/register', registerLimiter, async (req, res) => {
    try {
        ...
        if (existing) return res.json({ success: false, error: 'Registration failed. Please try a different username.' });
...
io.on('connection', (socket) => {
    ...
    // ⭐ Socket error handler (must exist or `socket.use` errors are silent)
    socket.on('error', (err) => {
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔌 Socket.IO: polling + websocket transports enabled`);
