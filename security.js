// security.js
// ⭐⭐⭐ Advanced Security Module v4.2 — Stable + Whitelist ⭐⭐⭐
...
const WHITELISTED_USERS = new Set([
    ...(process.env.WHITELIST_USERS || 'admin')
        .split(',')
...
const AUTOBAN_ENABLED = process.env.AUTOBAN_ENABLED === 'true';
...
function signTokenWithFingerprint(payload, fingerprint, secret) {
    ...
    const exp = now + (30 * 24 * 60 * 60); // ⭐ 30 يوم بدل 7
...
// ⭐ حدود متسامحة جداً — لن تزعج أحد
const limiters = {
    api:          new SlidingWindowLimiter(60 * 1000, 1000),
    apiUser:      new SlidingWindowLimiter(60 * 1000, 600),
    login:        new SlidingWindowLimiter(15 * 60 * 1000, 50),
    loginUser:    new SlidingWindowLimiter(15 * 60 * 1000, 30),
    register:     new SlidingWindowLimiter(60 * 60 * 1000, 30),
    roomMsg:      new SlidingWindowLimiter(10 * 1000, 50),
    privateMsg:   new SlidingWindowLimiter(10 * 1000, 50),
    gift:         new SlidingWindowLimiter(60 * 1000, 60),
    boost:        new SlidingWindowLimiter(60 * 1000, 20),
    game:         new SlidingWindowLimiter(60 * 1000, 120),
    call:         new SlidingWindowLimiter(5 * 60 * 1000, 20),
    friendReq:    new SlidingWindowLimiter(60 * 60 * 1000, 100),
    roomCreate:   new SlidingWindowLimiter(60 * 60 * 1000, 20),
    kick:         new SlidingWindowLimiter(60 * 1000, 60),
    socketConnect: new SlidingWindowLimiter(60 * 1000, 100),
    socketEvent:   new TokenBucket(1000, 100)   // ⭐ تم الرفع
};
...
function recordIpViolation(ip, reason) {
    ...
    if (AUTOBAN_ENABLED) {
        if (entry.violations >= 500) {
            entry.blocked = true;
            entry.blockedUntil = Date.now() + 24 * 60 * 60 * 1000;
        } else if (entry.violations >= 300) {
            entry.blocked = true;
            entry.blockedUntil = Date.now() + 60 * 60 * 1000;
        } else if (entry.violations >= 150) {
            entry.blocked = true;
...
function isWhitelisted(ip, username) {
    return isWhitelistedIP(ip) || isWhitelistedUser(username);
}

function isRateLimitExempt(ip) {
    return RATE_LIMIT_EXEMPT_IPS.has(ip) || isWhitelistedIP(ip);
}
...
// ═══════════════════════════════════════════════════════════
// 🚫 12. AUTO-BAN SYSTEM — VERY TOLERANT
// ═══════════════════════════════════════════════════════════
const VIOLATION_CONFIG = {
    'fingerprint_mismatch': { weight: 60, autoBan: false, banDuration: 3600, category: 'impersonation', severity: 'critical' },
    'socket_fp_mismatch':   { weight: 60, autoBan: false, banDuration: 3600, category: 'impersonation', severity: 'critical' },
    'bad_action_signature': { weight: 50, autoBan: false, banDuration: 3600, category: 'impersonation', severity: 'critical' },
    'targeted_bruteforce':  { weight: 40, autoBan: false, banDuration: 900,  category: 'bruteforce', severity: 'high' },
    'bruteforce_lockout':   { weight: 20, autoBan: false, banDuration: 900,  category: 'bruteforce', severity: 'medium' },
    'login_flood':          { weight: 10, autoBan: false, banDuration: 300,  category: 'flood', severity: 'medium' },
    'register_flood':       { weight: 15, autoBan: false, banDuration: 600,  category: 'flood', severity: 'medium' },
    'api_flood':            { weight: 5,  autoBan: false, banDuration: 120,  category: 'flood', severity: 'low' },
    'connection_flood':     { weight: 15, autoBan: false, banDuration: 600,  category: 'flood', severity: 'medium' },
    'invalid_token':        { weight: 10, autoBan: false, banDuration: 120,  category: 'auth', severity: 'low' },
    'flood_roomMsg':        { weight: 3,  autoBan: false, banDuration: 60,   category: 'flood', severity: 'low' },
    'flood_privateMsg':     { weight: 3,  autoBan: false, banDuration: 60,   category: 'flood', severity: 'low' },
    'flood_game':           { weight: 5,  autoBan: false, banDuration: 120,  category: 'flood', severity: 'low' },
    'flood_gift':           { weight: 5,  autoBan: false, banDuration: 120,  category: 'flood', severity: 'low' },
    'flood_boost':          { weight: 5,  autoBan: false, banDuration: 300,  category: 'flood', severity: 'low' },
    'flood_friendReq':      { weight: 5,  autoBan: false, banDuration: 120,  category: 'flood', severity: 'low' },
    'flood_roomCreate':     { weight: 8,  autoBan: false, banDuration: 300,  category: 'flood', severity: 'medium' },
    'flood_kick':           { weight: 5,  autoBan: false, banDuration: 120,  category: 'flood', severity: 'low' },
    'socket_rate_limit':    { weight: 2,  autoBan: false, banDuration: 30,   category: 'rate_limit', severity: 'low' },
    'unknown':              { weight: 2,  autoBan: false, banDuration: 30,   category: 'other', severity: 'low' }
};

const AUTO_BAN_THRESHOLDS = {
    levels: [
        { score: 200,  duration: 5 * 60,          reason: 'نقاط مشبوهة (200+)' },
        { score: 400,  duration: 30 * 60,         reason: 'نقاط مشبوهة (400+)' },
        { score: 700,  duration: 2 * 60 * 60,     reason: 'نقاط مشبوهة (700+)' },
        { score: 1000, duration: 6 * 60 * 60,     reason: 'نقاط مشبوهة (1000+)' },
        { score: 2000, duration: 24 * 60 * 60,    reason: 'نقاط مشبوهة (2000+)' }
    ],
    windowMs: 5 * 60 * 1000,
    decayRate: 0.3
};
...
function handleViolation(ip, type, username = null, extra = {}) {
    ...
    if (isWhitelisted(ip, username)) {
        logActivity({
            ...
            violationType: type, ip, username
        });
...
// ═══════════════════════════════════════════════════════════
// 🧹 15. MEMORY CLEANUP — مهم لمنع البطء
// ═══════════════════════════════════════════════════════════
setInterval(() => {
    const now = Date.now();
    ...
    // ⭐ suspicious + ipReputation — كل ساعة
    const HOUR = 60 * 60 * 1000;
    const MAX_AGE = 24 * HOUR;

    for (const [key, entry] of suspiciousActivity) {
        if (now - entry.lastSeen > MAX_AGE) suspiciousActivity.delete(key);
    }

    for (const [ip, entry] of ipReputation) {
        const lastTime = entry.reasons[entry.reasons.length - 1]?.time || 0;
        if (!entry.blocked && now - lastTime > MAX_AGE) {
    ...
    for (const [user, ips] of userIPs) {
        if (!ips || ips.length === 0) userIPs.delete(user);
    }
}, 60 * 1000);
