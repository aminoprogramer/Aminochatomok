...
// Auto-Ban + Whitelist + Toggle

const crypto = require('crypto');

// ============================================================
// 🛡️ 0. WHITELIST SYSTEM (يُقرأ من ENV)
// ============================================================
...
// ⭐ قائمة IPs معفاة من Rate Limiting أيضاً
...
// ⭐ فحص AUTO-BAN من ENV
const AUTOBAN_ENABLED = process.env.AUTOBAN_ENABLED !== 'false';
...
function verifyTokenWithFingerprint(token, currentFingerprint, secret) {
    try {
        ...
        // ⭐ لو المستخدم في whitelist، تخطى فحص البصمة
...
class SlidingWindowLimiter {
    ...
    check(key) {
        // ⭐ لو المفتاح في whitelist، تخطى الحد
...
function recordIpViolation(ip, reason) {
    ...
    // ⭐ الحظر التدريجي فقط لو Auto-Ban مفعّل
    if (AUTOBAN_ENABLED) {
...
function handleViolation(ip, type, username = null, extra = {}) {
    // ⭐⭐⭐ 1. فحص AUTO-BAN من ENV
    if (!AUTOBAN_ENABLED) {
        console.log(`⚠️ Auto-Ban DISABLED — ignoring: ${type} from ${ip}`);
        logActivity({
    ...
    if (isWhitelisted(ip, username)) {
        console.log(`✅ Whitelisted: ${ip}/${username} — ${type} IGNORED`);
        logActivity({
