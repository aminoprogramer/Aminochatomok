...
const translations = {
    en: {
        ...
        vip_room: "VIP Room (1500 coins)", cancel: "Cancel", create: "Create",
        ...
        get_question: "Get Question", new_password: "New Password", reset: "Reset",
        ...
        ban_user: "Ban User", banned_list: "Banned Users", upload_image: "Upload Image",
        ...
        logout: "Logout", menu: "Menu", vip_status: "VIP Status", buy_vip: "Buy VIP",
        ...
        basic_boost: "Basic", silver_boost: "Silver", gold_boost: "Gold",
        create_room_title: "Create Room (1500 coins)", vip_room_check: "VIP Room (gold theme + YouTube)",
        send_gift_title: "Send Gift", request_coins_title: "Request Coins",
        ...
        select_gender: "Select Gender", get_question_btn: "Next",
    ...
    ar: {
        ...
        vip_room: "غرفة VIP (1500 عملة)", cancel: "إلغاء", create: "إنشاء",
        ...
        get_question: "الحصول على السؤال", new_password: "كلمة المرور الجديدة", reset: "إعادة تعيين",
        ...
        ban_user: "طرد مستخدم", banned_list: "المطرودون", upload_image: "رفع صورة",
        ...
        menu: "القائمة", vip_status: "حالة VIP", buy_vip: "شراء VIP",
        ...
        basic_boost: "أساسي", silver_boost: "فضي", gold_boost: "ذهبي",
        create_room_title: "إنشاء غرفة (1500 عملة)", vip_room_check: "غرفة VIP (ثيم ذهبي + يوتيوب)",
        send_gift_title: "إرسال هدية", request_coins_title: "طلب عملات",
        ...
        select_gender: "اختر الجنس", get_question_btn: "التالي",
...
function refreshAllDynamicViews() {
    ...
    if (document.getElementById('friendsView')?.classList.contains('active')) {
        loadFriends();
        loadFriendRequests();
    }
    if (document.getElementById('giftsView')?.classList.contains('active')) {
        loadGiftHistory();
        loadTargetInfo();
        loadPayoutHistory();
    }
    if (document.getElementById('gamesView')?.classList.contains('active')) {
        loadGameHistory();
    }
...
function updateDynamicContent() {
    ...
    if (vipDisplay) {
        ...
        if (currentText.includes('Active until') || currentText.includes('نشط حتى')) {
            ...
            if (dateMatch) vipDisplay.innerText = `💎 ${t('vip_status')}: ${dateMatch[1]}`;
...
function addNotification(title, message, onClickData) {
    ...
    if (title === t('new_message') || title === t('friend_request') || title === t('gift_received') || title === t('support_reply') || title === t('ticket_resolved'))
...
function updateNotificationUI() {
    ...
    if (!container) return;
    if (!notifications.length) container.innerHTML = `<div style="padding:15px;text-align:center;color:#999;">${t('no_notifications') || 'No notifications'}</div>`;
...
async function register() {
    ...
    if (!username || !password || !securityQuestion || !securityAnswer) return alert(t('fill_required') || 'All fields required');
    ...
    const data = await res.json();
    if (data.success) { alert(t('registered_ok') || 'Registered! Please login.'); showLogin(); } else alert('Registration failed: ' + data.error);
}
...
async function getSecurityQuestion() {
    ...
    try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            if (text.includes('<!DOCTYPE html>')) throw new Error('Server returned HTML');
...
async function resetPassword() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            statusDiv.innerHTML = '✅ ' + (t('password_reset_ok') || 'Password reset! You can now login.');
...
async function uploadProfilePic() {
    ...
    input.onchange = async (e) => {
        ...
        reader.onload = async (ev) => {
            ...
            } catch (err) { alert((t('upload_error') || 'Upload error') + ': ' + err.message); }
        };
...
setInterval(() => { if (currentPrivateChat) updatePrivateStatus(); }, 5000);
...
async function translateAndDisplayMessage(msg, containerId, isSentByMe) {
    ...
    if (msg.type === 'image') {
        ...
        if (autoTranslateMessages && targetLanguage !== 'en') {
            ...
            } catch (err) { content = escapeHtml(msg.message); }
        } else {
            content = escapeHtml(msg.message);
        }
...
async function loadTargetInfo() {
    ...
    try {
        ...
        if (reward) reward.textContent = `💰 ${t('earned')}: $${data.dollarsEarned} | ${t('total') || 'Total'}: $${data.totalPaid || 0}`;

        if (streakBadge) {
            streakBadge.textContent = `🔥 ${streak}/10 ${t('days') || 'days'}`;
        ...
        if (!payouts.length) {
            ...
            cont.innerHTML = payouts.map(p => `
                <div class="payout-item">
                    <strong>${p.month}</strong> · ${p.targetAmount.toLocaleString()} ${t('target_label') || 'target'} →
...
async function sendRoomGift() {
    ...
    if (!to) return alert(t('select_user') || 'Select a user');
    if (!amount) return alert(t('select_amount') || 'Select amount');
    ...
    const data = await res.json();
    if (data.success) {
        showToast(`🎁 Gift sent to ${to}! +${data.targetContribution} target added (no coins)`);
        closeRoomGiftModal();
        updateCoins();
        loadGiftHistory();
        if (currentRoom) {
...
async function loadFriendRequests() {
    ...
    if (!requests.length) container.innerHTML = `<div>${t('no_requests') || 'No requests'}</div>`;
...
async function deleteFriend(friend) {
    ...
    try {
        ...
        else alert('Failed to remove friend: ' + (data.error || 'Unknown error'));
...
async function searchUsers() {
    ...
    if (!filtered.length) container.innerHTML = `<div>${t('no_users_found') || 'No users found'}</div>`;
...
async function updateCoins() {
    ...
    const data = await res.json();
    if (data.success) { currentUser.coins = data.user.coins; document.getElementById('coinBalance').innerHTML = `💰 ${currentUser.coins}`; updateGamesBalance(); }
}
...
function boostRoom() { const amount = parseInt(document.getElementById('boostAmount').value); if (amount && currentRoom && currentUser.coins >= amount) { socket.emit('boost-room', { username: currentUser.username, roomName: currentRoom, amount }); closeBoostModal(); updateCoins(); } else alert(t('insufficient_coins') || 'Insufficient coins'); }
...
async function getCallConfig() {
    ...
    if (turnServers && turnServers.length > 0) {
        ...
        iceServers.push({ urls: 'turn:numb.viagenie.ca', username: 'webrtc@live.com', credential: 'muazkh' });
    }
...
function setupPeerConnectionListeners(pc, isCaller = false) {
    ...
    pc.ontrack = (event) => {
        ...
        const playRemote = () => {
            remoteVideo.play().catch(err => {
                setTimeout(() => remoteVideo.play().catch(e => console.error(e)), 500);
            });
        };
        playRemote();
    ...
    pc.oniceconnectionstatechange = () => {
        if (pc.iceConnectionState === 'failed') { showToast('Call connection failed (ICE)'); endCall(); }
        else if (pc.iceConnectionState === 'connected') showToast('Call connected');
        else if (pc.iceConnectionState === 'disconnected') showToast('Call disconnected');
    };
    pc.onconnectionstatechange = () => {
        if (pc.connectionState === 'failed') { showToast('Call connection failed'); endCall(); }
...
async function startCall() {
    ...
    try {
        ...
        if (!localStream.getAudioTracks().length) showToast('⚠️ No microphone');
...
async function acceptCall() {
    ...
    try {
        ...
        const config = await getCallConfig();
        const constraints = { audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true }, video: { facingMode: 'user', width: { ideal: 640 }, height: { ideal: 480 } } };
        localStream = await navigator.mediaDevices.getUserMedia(constraints);
        if (!localStream.getAudioTracks().length) showToast('⚠️ No microphone');
...
async function toggleSpeaker() {
    ...
    try {
        ...
        let targetSinkId;
        if (!speakerEnabled) {
            const speakerDevice = audioOutputs.find(d => d.label.toLowerCase().includes('speaker') || d.label.toLowerCase().includes('headphone'));
            targetSinkId = speakerDevice ? speakerDevice.deviceId : audioOutputs.find(d => d.deviceId !== 'default')?.deviceId;
            if (!targetSinkId) { showToast('No speaker device found'); return; }
        } else {
...
function toggleCallAudio() {
    ...
    } else showToast('No audio track found');
}
...
function toggleCallVideo() {
    ...
    } else showToast('No video track found');
}
        ...
        fetch(`https://noembed.com/embed?url=https://www.youtube.com/watch?v=${encodeURIComponent(query)}`)
            ...
            .then(data => {
                if (data.url) {
                    ...
                    else { window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`, '_blank'); showToast('Opening YouTube search'); }
...
async function showAllUsers() { const res = await fetch(`/api/admin/get-all-users?admin=${currentUser.username}`); const users = await res.json(); if (users.error) { alert('Admin access required'); return; } const html = users.map(u => `<div>${escapeHtml(u.username)} (💰 ${u.coins}) ${u.isAdmin ? '👑 Admin' : ''} <button onclick="deleteUser('${escapeHtml(u.username)}')">${t('delete')}</button></div>`).join(''); document.getElementById('allUsersList').innerHTML = html; document.getElementById('usersModal').classList.add('active'); }
...
async function showAllRooms() { const res = await fetch(`/api/get-all-rooms?admin=${currentUser.username}`); const rooms = await res.json(); const html = rooms.map(r => `<div>${r.name} (Owner: ${r.owner}) <button onclick="deleteRoom('${r.name}')">${t('delete')}</button></div>`).join(''); document.getElementById('allRoomsList').innerHTML = html; document.getElementById('roomsModal').classList.add('active'); }
...
async function showPendingManualRequests() { const res = await fetch(`/api/get-pending-manual-requests?admin=${currentUser.username}`); const reqs = await res.json(); const html = reqs.map(r => `<div>${r.username} requested ${r.amount} coins (${r.paymentMethod}) <button onclick="approveRequest('${r._id}')">${t('approve')}</button> <button onclick="rejectRequest('${r._id}')">${t('reject')}</button> ${r.proof ? `<a href="${r.proof}" target="_blank">Proof</a>` : ''}</div>`).join(''); document.getElementById('pendingRequestsList').innerHTML = html; document.getElementById('pendingRequestsModal').classList.add('active'); }
...
async function submitSupportTicket() {
    ...
    if (!subject || !message) { alert(t('fill_required') || 'Please fill subject and message.'); return; }
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) { responseDiv.innerHTML = `<span style="color:green;">✅ ${t('ticket_submitted') || 'Ticket submitted!'} ID: ${data.ticketId}</span>`; setTimeout(() => closeSupportModal(), 2000); }
        else responseDiv.innerHTML = `<span style="color:red;">❌ ${data.error || 'Unknown error'}</span>`;
    } catch (err) { responseDiv.innerHTML = `<span style="color:red;">❌ ${t('network_error_msg') || 'Network error.'}</span>`; }
}
...
async function showAdminSupportPanel() {
    ...
    try {
        ...
        if (!res.ok || !Array.isArray(data)) {
            ...
            container.innerHTML = `<div style="padding:12px;color:#ef4444;">❌ ${escapeHtml(msg)}</div>`;
            return;
        }
        ...
        if (!data.length) {
            container.innerHTML = `<div style="padding:12px;text-align:center;color:#6b7280;">${t('no_tickets')}</div>`;
            return;
        }
        ...
        container.innerHTML = data.map(ticket => `
            <div class="ticket-item">
                ...
                ${ticket.screenshot ? `<a href="${escapeHtml(ticket.screenshot)}" target="_blank" rel="noopener">${t('view_screenshot')}</a><br>` : ''}
                ...
                <div style="margin-top:10px;">
                    ...
                    ${ticket.status !== 'resolved'
                        ? `<button onclick="resolveTicket('${escapeHtml(ticket.ticketId)}')">${t('resolve_btn')}</button>`
                        : ''}
                </div>
            </div>
        `).join('');
    } catch (err) {
        console.error('showAdminSupportPanel error:', err);
...
async function sendSupportReply() {
    ...
    try {
        ...
        else alert('Failed to send reply: ' + (data.error || 'Unknown error'));
    } catch (err) { alert('Network error: ' + err.message); }
}
...
function refreshRoomMessagesModeratorStatus() {
    ...
    messages.forEach(wrapper => {
        ...
        if (isOwner) senderDiv.classList.add('owner');
        else if (isMod) {
...
function initSocket() {
    ...
    socket.on('room-messages-history', (data) => { });
    ...
    socket.on('coins-updated', (balance) => { currentUser.coins = balance; document.getElementById('coinBalance').innerHTML = `💰 ${balance}`; updateGamesBalance(); });
    ...
    socket.on('target-updated', (data) => {
        showToast(`🎯 +${data.contribution} ${t('target')} ${t('from')} ${data.from} | ${t('total') || 'Total'}: ${data.monthlyTarget.toLocaleString()} ($${data.dollarsEarned})`);
    ...
    socket.on('streak-completed', (data) => {
        ...
        addNotification('🎉 10 Days Complete!', `You earned $${data.dollarsEarned}!`);
        showToast(`🎉 10 days reached! Payout: $${data.dollarsEarned}`);
        loadTargetInfo();
        loadPayoutHistory();
    });
    ...
    socket.on('target-not-reached', (data) => {
        playNotificationSound();
        const remaining = data.minTarget - data.currentTarget;
        showToast(`⚠️ 10 days done, but you need ${remaining.toLocaleString()} more target`);
    ...
    socket.on('monthly-reset', (data) => {
        playNotificationSound();
        addNotification('📅 Monthly Reset', `Target reset (${data.reason})`);
        showToast(`📅 ${data.reason} — Target reset`);
        loadTargetInfo();
    });
...
async function sendManualCoinRequest() {
    ...
    const data = await res.json();
    if (data.success) alert(t('request_submitted') || 'Request submitted! Admin will review.');
    else alert('Error');
...
async function purchaseCoins(coins, name, price, method) {
    ...
    if (method === 'nowpayment') {
        ...
        alert('⚠️ Card payment is not available. Please use Crypto (NOWPayments) or "Request Coins (Manual)".');
    }
}

async function startCryptoPayment(packageId) {
    try {
        showToast(t('creating_invoice') || 'Creating crypto invoice...');
        ...
        if (!win) showToast(t('popup_blocked') || 'Please allow pop-ups to complete payment');
    ...
    const tick = async () => {
        ...
        try {
            ...
            if (data.credited || data.status === 'finished' || data.status === 'confirmed') {
                ...
                addNotification('💰 ' + t('payment_confirmed'), `${data.coins} coins (${orderId})`);
                await updateCoins();
                return;
            }
            ...
            if (['failed', 'expired', 'refunded'].includes(data.status)) {
                ...
                addNotification('❌ Payment ' + data.status, `Order ${orderId}`);
                return;
            }
        } catch (_) { }
...
function handlePaymentReturn() {
    ...
    if (payment === 'success' && order) {
        showToast('⏳ ' + (t('verifying_payment') || 'Verifying payment...'));
        pollCryptoPayment(order, true);
    } else if (payment === 'cancel') {
        showToast(t('payment_cancelled') || 'Payment cancelled');
    }
}
