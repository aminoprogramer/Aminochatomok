...
async function getTurnCredentials() {
    ...
    try {
        const response = await authFetch('/api/turn-credentials');
...
const translations = {
    en: {
        ...
        ip_ban_dashboard: "IP Ban Dashboard",
        total_bans: "Total Bans",
        active_bans: "Active",
        blocked_today: "Blocked (24h)",
        tab_bans: "Bans",
        tab_whitelist: "Whitelist",
        tab_logs: "Activity Logs",
        ban_ip: "Ban IP",
        add_whitelist: "Whitelist",
        banned_notification: "Your IP has been banned"
    },
    ar: {
        ...
        ip_ban_dashboard: "لوحة حظر IP",
        total_bans: "إجمالي الحظرات",
        active_bans: "نشط",
        blocked_today: "محظور (24 ساعة)",
        tab_bans: "الحظرات",
        tab_whitelist: "القائمة البيضاء",
        tab_logs: "سجل النشاط",
        ban_ip: "حظر IP",
        add_whitelist: "إضافة للقائمة البيضاء",
        banned_notification: "تم حظر عنوان IP الخاص بك"
    }
};
...
function applyLanguage() {
    ...
    if (activeId) {
        ...
        if (el && document.activeElement !== el) {
            try {
                el.focus();
                if (selectionStart !== null && selectionStart !== undefined && el.setSelectionRange) {
...
async function refreshUserData(force = false) {
    ...
    try {
        const res = await authFetch(`/api/user-profile?username=${currentUser.username}`);
        const data = await res.json();
        if (data.success) {
...
async function checkAndUpdateVIPStatus() {
    ...
    try {
        const res = await authFetch(`/api/get-vip-status`);
        const data = await res.json();
...
async function uploadProfilePic() {
    ...
    input.onchange = async (e) => {
        ...
        reader.onload = async (ev) => {
            ...
            try {
                const res = await authFetch('/api/upload-profile-pic-base64', {
                    method: 'POST',
...
async function adminLookupPrivateCode() {
    ...
    try {
        ...
        if (data.success) {
            ...
            btn.textContent = '📋';
            btn.onclick = () => {
                navigator.clipboard.writeText(data.privateCode);
                showToast('📋 Copied');
            };
            el.appendChild(btn);
        } else {
...
function renderActiveRooms(rooms) {
    ...
    container.innerHTML = active.map(r => {
        ...
        return `<div class="active-room-chip ${vipClass}" data-room="${escapeHtml(r.name)}">
    ...
    container.querySelectorAll('.active-room-chip').forEach(chip => {
        chip.addEventListener('click', () => joinRoom(chip.dataset.room));
    });
}
...
function displayRooms(rooms) {
    ...
    if (!rooms.length) {
        ...
        sorted.forEach((room, i) => {
            ...
            parts.push(`<div class="room-card ${isTop ? 'boosted' : ''}" data-room="${escapeHtml(room.name)}"><div class="room-card-info"><h4 class="${nameClass}">#${escapeHtml(room.name)}${vipTag}${membersOnlyTag} ${room.boostLevel > 0 ? '🚀' : ''}</h4><p>👥 ${room.members} ${t('members')} | 🚀 ${t('boost')}: ${room.boostLevel}</p></div><button class="join-btn" data-room="${escapeHtml(room.name)}">${t('join')}</button></div>`);
        });
...
async function translateAndDisplayMessage(msg, containerId, isSentByMe) {
    ...
    if (msg.type === 'image') {
        content = `<img src="${safeMsg}" class="message-img" data-lightbox="${safeMsg}">`;
    ...
    div.innerHTML = `<div class="message-avatar" data-user="${escapeHtml(username)}">${avatarHtml}</div>
    ...
    if (avatarEl) {
        avatarEl.addEventListener('click', () => showUserProfile(avatarEl.dataset.user));
    }
    const imgEl = div.querySelector('[data-lightbox]');
    if (imgEl) {
        imgEl.addEventListener('click', () => openLightbox(imgEl.dataset.lightbox));
    }
...
async function startRecording(target) {
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            const res = await authFetch('/api/upload-voice', { method: 'POST', body: fd });
            const data = await res.json();
            if (data.url) {
                if (target === 'room' && currentRoom) socket.emit('room-message', { message: data.url, room: currentRoom, type: 'audio' });
                else if (target === 'private' && currentPrivateChat) socket.emit('private-message', { to: currentPrivateChat, message: data.url, type: 'audio' });
            }
...
async function uploadImage() {
    ...
    const res = await authFetch('/api/upload-room-image', { method: 'POST', body: fd });
    const data = await res.json();
    if (data.success) {
        if (currentImageTarget === 'room' && currentRoom) socket.emit('room-message', { message: data.imageUrl, room: currentRoom, type: 'image' });
        else if (currentImageTarget === 'private' && currentPrivateChat) socket.emit('private-message', { to: currentPrivateChat, message: data.imageUrl, type: 'image' });
        closeImageModal();
...
async function loadGiftHistory() {
    const res = await authFetch('/api/get-gifts');
    ...
    else container.innerHTML = myGifts.map(g => `<div class="gift-history-item">🎁 ${escapeHtml(g.from)} → ${escapeHtml(g.to)}: ${g.amount} ${t('coins')}<br>💬 ${escapeHtml(g.message || '')}<br><small>${escapeHtml(g.timestamp)}</small></div>`).join('');
}

async function loadTargetInfo() {
    if (!currentUser) return;
    try {
        const res = await authFetch(`/api/get-target-info`);
        const data = await res.json();
...
async function loadPayoutHistory() {
    if (!currentUser) return;
    try {
        const res = await authFetch(`/api/get-payouts`);
        ...
        if (!payouts.length) {
            ...
            cont.innerHTML = payouts.map(p => `
                <div class="payout-item">
                    <strong>${escapeHtml(p.month)}</strong> · ${(p.targetAmount || 0).toLocaleString()} ${t('target_label')} →
                    <span style="color:#10b981;font-weight:bold;">$${p.dollarsEarned}</span>
                    <span class="payout-status ${escapeHtml(p.status)}">${escapeHtml(p.status)}</span>
                </div>
            `).join('');
        }
...
async function showRoomGiftModal() {
    ...
    try {
        const res = await authFetch(`/api/get-room-members/${currentRoom}`);
        const data = await res.json();
...
async function sendRoomGift() {
    ...
    const res = await authFetch('/api/send-gift', {
        method: 'POST',
...
async function showAdminPayouts() {
    const res = await authFetch(`/api/get-payouts?admin=${currentUser.username}`);
    const payouts = await res.json();
    const html = payouts.length ? payouts.map(p => `
        <div class="payout-item">
            <strong>${escapeHtml(p.username)}</strong> · ${escapeHtml(p.month)} · ${(p.targetAmount || 0).toLocaleString()} → 
            <b>$${p.dollarsEarned}</b>
            <span class="payout-status ${escapeHtml(p.status)}">${escapeHtml(p.status)}</span>
            ${p.status !== 'paid' ? `<button class="admin-btn green" style="margin-left:8px;" data-payout-id="${escapeHtml(String(p._id))}">${t('mark_paid')}</button>` : ''}
        </div>
    `).join('') : `<div>${t('no_payouts')}</div>`;
    const container = document.getElementById('adminPayoutsList');
    container.innerHTML = html;
    container.querySelectorAll('[data-payout-id]').forEach(btn => {
        btn.addEventListener('click', () => markPayoutPaid(btn.dataset.payoutId));
    });
    document.getElementById('adminPayoutsModal').classList.add('active');
}

async function markPayoutPaid(id) {
    await authFetch('/api/mark-payout-paid', {
        method: 'POST',
...
async function loadFriends() {
    const res = await authFetch(`/api/get-friends`);
    ...
    if (!friends.length) container.innerHTML = `<div>${t('no_friends')}</div>`;
        ...
        return `<div class="room-card"><div class="room-card-info"><h4>${escapeHtml(f)}</h4><p>${statusText}</p></div><div class="friend-actions"><button class="delete-friend-btn" data-user="${escapeHtml(f)}">${t('remove')}</button><button class="join-btn" data-chat="${escapeHtml(f)}">${t('chat')}</button></div></div>`;
    }).join('');

    container.querySelectorAll('[data-user]').forEach(btn => {
        btn.addEventListener('click', () => deleteFriend(btn.dataset.user));
    });
    container.querySelectorAll('[data-chat]').forEach(btn => {
        btn.addEventListener('click', () => showPrivateChat(btn.dataset.chat));
    });
}

async function loadFriendRequests() {
    const res = await authFetch(`/api/get-friend-requests`);
    const requests = await res.json();
    const container = document.getElementById('friendRequestsList');
    if (!requests.length) container.innerHTML = `<div>${t('no_requests')}</div>`;
    else container.innerHTML = requests.map(req => `<div class="friend-request-item">${escapeHtml(req.from)} <button data-accept="${escapeHtml(req.from)}">${t('accept')}</button></div>`).join('');
...
async function deleteFriend(friend) {
    ...
    try {
        const res = await authFetch('/api/delete-friend', {
            method: 'POST',
...
async function searchUsers() {
    ...
    if (!q) return;
    const res = await authFetch('/api/get-all-users');
    ...
    else container.innerHTML = filtered.map(u => `<div class="search-result-item">${escapeHtml(u.username)} <button class="send-req-btn" data-user="${escapeHtml(u.username)}">${t('add_friend')}</button></div>`).join('');

    container.querySelectorAll('[data-user]').forEach(btn => {
        btn.addEventListener('click', () => sendFriendRequest(btn.dataset.user));
    });
}
...
async function playGame(game, choice) {
    ...
    try {
        const res = await authFetch('/api/game/play', {
            method: 'POST',
...
async function loadGameHistory() {
    if (!currentUser) return;
    try {
        const res = await authFetch(`/api/game/history?limit=15`);
        const data = await res.json();
...
async function showAdminGameLogs() {
    const res = await authFetch(`/api/game/admin-logs`);
...
async function buyVIP() {
    const res = await authFetch('/api/buy-vip', { method: 'POST' });
    const data = await res.json();
    if (data.success) {
...
async function loadRoomMembers() {
    ...
    if (data.success) {
        ...
        for (const member of data.members) {
            ...
            parts.push(`<div class="user-item"><div class="user-avatar" data-user="${escapeHtml(member)}">${avatarHtml}</div><div><span ${nameColor}>${escapeHtml(member)}</span>${isVIP ? ' 💎' : ''}${roleBadge}</div>${showKick ? `<button class="kick-btn" data-kick="${escapeHtml(member)}">${t('kick')}</button>` : ''}</div>`);
        }
        container.innerHTML = parts.join('');
        ...
        container.querySelectorAll('[data-kick]').forEach(btn => {
            btn.addEventListener('click', () => kickUserFromRoom(btn.dataset.kick));
        });
        ...
        if ((currentUser.username === currentRoomOwner || currentRoomModerators.includes(currentUser.username)) && data.kicked && data.kicked.length) {
            kickedCont.innerHTML = `<h4>${t('banned_users')}</h4>` + data.kicked.map(k => `<div class="user-item"><span>${escapeHtml(k)}</span><button class="unban-btn" data-unban="${escapeHtml(k)}">${t('unban')}</button></div>`).join('');
            kickedCont.querySelectorAll('[data-unban]').forEach(btn => {
                btn.addEventListener('click', () => unbanUserFromRoom(btn.dataset.unban));
            });
...
async function loadMemberProfiles(members) {
    await Promise.all(members.map(async (m) => {
        ...
        try {
            const res = await authFetch(`/api/user-profile?username=${m}`);
            const data = await res.json();
            if (data.success) {
                profileCache.set(m, data);
                setTimeout(() => {
                    const items = document.querySelectorAll('.user-item .user-avatar');
                    const target = Array.from(items).find(el => el.dataset.user === m);
...
async function kickUserFromRoom(username) {
    ...
    const res = await authFetch('/api/kick-user', {
        method: 'POST',
...
async function unbanUserFromRoom(username) {
    const res = await authFetch('/api/unban-user', {
        method: 'POST',
        body: JSON.stringify({ roomName: currentRoom, username })
    });
...
async function showRoomSettings() {
    ...
    const res = await authFetch(`/api/room-settings/${currentRoom}`);
    const data = await res.json();
    if (data.success) {
...
function renderMemberManageList(allowed, moderators, isOwner) {
    ...
    cont.innerHTML = allowed.map(m => {
        ...
        return `<div class="user-item"><div class="user-avatar">👤</div><div><strong>${escapeHtml(m)}</strong> ${isMod ? '<span class="moderator-badge">MOD</span>' : ''}</div><div style="margin-left:auto;">${isOwner ? (isMod ? `<button class="kick-btn" data-remove-mod="${escapeHtml(m)}">${t('remove_mod')}</button>` : `<button class="mod-btn" data-set-mod="${escapeHtml(m)}">${t('make_mod')}</button>`) : ''}<button class="kick-btn" data-remove-member="${escapeHtml(m)}">${t('remove')}</button></div></div>`;
    }).join('');

    cont.querySelectorAll('[data-remove-mod]').forEach(btn => btn.addEventListener('click', () => removeModerator(btn.dataset.removeMod)));
    cont.querySelectorAll('[data-set-mod]').forEach(btn => btn.addEventListener('click', () => setModerator(btn.dataset.setMod)));
    cont.querySelectorAll('[data-remove-member]').forEach(btn => btn.addEventListener('click', () => removeRoomMember(btn.dataset.removeMember)));
}
...
async function toggleMembersOnly() {
    ...
    const res = await authFetch('/api/room-settings', {
        method: 'POST',
...
function setModerator(username) { socket.emit('set-moderator', { roomName: currentRoom, username }); }
function removeModerator(username) { socket.emit('remove-moderator', { roomName: currentRoom, username }); }
    ...
    pc.onnegotiationneeded = async () => {
        if (callActive && currentPrivateChat && socket?.connected) {
            try {
                ...
                socket.emit('call-user', { to: currentPrivateChat, offer: pc.localDescription });
...
async function startCall() {
    ...
    try {
        ...
        socket.emit('call-user', { to: currentPrivateChat, offer: peerConnection.localDescription });
...
async function showAllUsers() {
    const res = await authFetch(`/api/admin/get-all-users`);
    const users = await res.json();
    if (users.error) { alert('Admin access required'); return; }
    const html = users.map(u => `<div>${escapeHtml(u.username)} (💰 ${u.coins}) ${u.isAdmin ? '👑' : ''} ${u.privateCode ? `🔑 <code style="font-size:11px;">${escapeHtml(u.privateCode)}</code>` : ''} <button data-del-user="${escapeHtml(u.username)}">${t('delete')}</button></div>`).join('');
...
async function deleteUser(username) {
    if (confirm('Delete user?')) {
        await authFetch('/api/delete-user', {
            method: 'POST',
            body: JSON.stringify({ username })
        });
        showAllUsers();
...
async function showAllRooms() {
    const res = await authFetch(`/api/get-all-rooms`);
    const rooms = await res.json();
    const html = rooms.map(r => `<div>${escapeHtml(r.name)} (Owner: ${escapeHtml(r.owner)}) <button data-del-room="${escapeHtml(r.name)}">${t('delete')}</button></div>`).join('');
    const container = document.getElementById('allRoomsList');
    container.innerHTML = html;
    container.querySelectorAll('[data-del-room]').forEach(btn => {
        btn.addEventListener('click', () => deleteRoom(btn.dataset.delRoom));
    });
    document.getElementById('roomsModal').classList.add('active');
}
async function deleteRoom(roomName) {
    if (confirm('Delete room?')) {
        await authFetch('/api/delete-room', {
            method: 'DELETE',
...
async function showAllGifts() {
    const res = await authFetch('/api/get-gifts');
    const gifts = await res.json();
    const html = gifts.map(g => `<div>${escapeHtml(g.from)} → ${escapeHtml(g.to)}: ${g.amount} coins (${escapeHtml(g.timestamp)})</div>`).join('');
...
async function clearAllFriendRequests() {
    if (confirm('Clear all friend requests?')) {
        await authFetch('/api/clear-friend-requests', { method: 'POST' });
        alert('Cleared');
    }
}
async function clearAllCoinRequests() {
    if (confirm('Clear coin history?')) {
        await authFetch('/api/clear-coin-requests', { method: 'POST' });
        alert('Cleared');
    }
}
async function showBoostLogModal() {
    const res = await authFetch('/api/get-boost-logs');
    const logs = await res.json();
    const html = logs.map(l => `<div>${escapeHtml(l.username)} boosted ${escapeHtml(l.roomName)} with ${l.amount} coins</div>`).join('');
...
async function showPendingManualRequests() {
    const res = await authFetch(`/api/get-pending-manual-requests`);
    const reqs = await res.json();
    const html = reqs.map(r => `<div>${escapeHtml(r.username)} requested ${r.amount} coins (${escapeHtml(r.paymentMethod)}) <button data-approve="${r._id}">${t('approve')}</button> <button data-reject="${r._id}">${t('reject')}</button> ${r.proof ? `<a href="${escapeHtml(r.proof)}" target="_blank">Proof</a>` : ''}</div>`).join('');
    const container = document.getElementById('pendingRequestsList');
    container.innerHTML = html;
    container.querySelectorAll('[data-approve]').forEach(btn => btn.addEventListener('click', () => approveRequest(btn.dataset.approve)));
    container.querySelectorAll('[data-reject]').forEach(btn => btn.addEventListener('click', () => rejectRequest(btn.dataset.reject)));
    document.getElementById('pendingRequestsModal').classList.add('active');
}
async function approveRequest(id) {
    await authFetch('/api/approve-manual-request', { method: 'POST', body: JSON.stringify({ requestId: id }) });
    showPendingManualRequests();
}
async function rejectRequest(id) {
    await authFetch('/api/reject-manual-request', { method: 'POST', body: JSON.stringify({ requestId: id }) });
    showPendingManualRequests();
}
async function adminGiveCoins() {
    ...
    await authFetch('/api/admin-give-coins', { method: 'POST', body: JSON.stringify({ username, amount }) });
    alert('Coins given');
}
async function adminResetPassword() {
    ...
    await authFetch('/api/admin-reset-password', { method: 'POST', body: JSON.stringify({ username, newPassword }) });
...
// ==================== ADMIN: IP BAN DASHBOARD ====================
let currentIpTab = 'bans';
let ipBanListCache = [];

async function showAdminIpDashboard() {
    if (!currentUser || !currentUser.isAdmin) return showToast('Admin access required');
    document.getElementById('adminIpDashboardModal').classList.add('active');
    await loadIpStats();
    switchIpTab('bans');
}

function closeAdminIpDashboard() {
    document.getElementById('adminIpDashboardModal').classList.remove('active');
}

function switchIpTab(tab) {
    currentIpTab = tab;
    document.querySelectorAll('.ip-tab').forEach(t => t.classList.toggle('active', t.dataset.ipTab === tab));
    document.querySelectorAll('.ip-tab-panel').forEach(p => p.classList.remove('active'));
    const panelId = `ipTab${tab.charAt(0).toUpperCase() + tab.slice(1)}`;
    document.getElementById(panelId)?.classList.add('active');

    if (tab === 'bans') loadIpBans();
    else if (tab === 'whitelist') loadIpWhitelist();
    else if (tab === 'logs') loadIpLogs();
}

async function loadIpStats() {
    try {
        const res = await authFetch('/api/admin/ip-stats');
        const data = await res.json();
        if (!data.success) return;
        document.getElementById('ipTotalBans').textContent = data.totalBans;
        document.getElementById('ipActiveBans').textContent = data.activeBans;
        const hits = (data.topFailed || []).reduce((s, t) => s + t.count, 0);
        document.getElementById('ipBannedHits').textContent = hits;
    } catch (e) { console.error('loadIpStats', e); }
}

async function loadIpBans() {
    const search = document.getElementById('ipSearchInput')?.value.trim() || '';
    const status = document.getElementById('ipStatusFilter')?.value || 'all';
    ...
    try {
        ...
        const res = await authFetch(`/api/admin/ip-bans?${params}`);
        const data = await res.json();

        if (!data.success || !data.bans.length) {
            container.innerHTML = '<div class="ip-empty">No bans found</div>';
            ipBanListCache = [];
            return;
        }

        ipBanListCache = data.bans;
        container.innerHTML = data.bans.map(b => {
            const isExpired = !b.permanent && b.expiresAt && new Date(b.expiresAt) <= new Date();
            const statusClass = isExpired ? 'expired' : (b.permanent ? 'permanent' : 'active');
            const statusLabel = isExpired ? 'EXPIRED' : (b.permanent ? 'PERMANENT' : 'ACTIVE');
            const expiryText = b.permanent ? '♾️ Never' : new Date(b.expiresAt).toLocaleString();
            return `
                <div class="ip-item ${statusClass}">
                    <div class="ip-item-header">
                        <code class="ip-address">${escapeHtml(b.ip)}</code>
                        <span class="ip-badge ${statusClass}">${statusLabel}</span>
                    </div>
                    <div class="ip-item-body">
                        <div><strong>Reason:</strong> ${escapeHtml(b.reason)}</div>
                        ${b.username ? `<div><strong>User:</strong> ${escapeHtml(b.username)}</div>` : ''}
                        <div><strong>Banned by:</strong> ${escapeHtml(b.bannedBy)}</div>
                        <div><strong>Expires:</strong> ${expiryText}</div>
                        <div><strong>Hits:</strong> ${b.hitCount || 0} ${b.lastHitAt ? `(last: ${new Date(b.lastHitAt).toLocaleString()})` : ''}</div>
                        <div class="ip-item-actions">
                            <button class="admin-btn green" data-unban="${escapeHtml(b.ip)}">✅ Unban</button>
                            <button class="admin-btn orange" data-view-logs="${escapeHtml(b.ip)}">📜 Logs</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        container.querySelectorAll('[data-view-logs]').forEach(btn => {
            btn.addEventListener('click', () => {
                switchIpTab('logs');
                const logSearchInput = document.getElementById('ipLogSearchInput');
                if (logSearchInput) logSearchInput.value = btn.dataset.viewLogs;
                loadIpLogs();
            });
        });
    } catch (e) {
        container.innerHTML = `<div class="ip-empty">❌ ${escapeHtml(e.message)}</div>`;
    }
}

async function banIp() {
    const ip = document.getElementById('ipBanInput').value.trim();
    const duration = document.getElementById('ipBanDuration').value;
    ...
    const body = { ip, reason: reason || 'Manual ban', permanent };
    if (!permanent) body.duration = parseInt(duration);
...
async function loadIpWhitelist() {
    ...
    try {
        ...
        const data = await res.json();
        if (!data.success || !data.whitelist.length) {
            container.innerHTML = '<div class="ip-empty">No whitelisted IPs</div>';
            return;
        }
        container.innerHTML = data.whitelist.map(w => `
            <div class="ip-item whitelist">
                <div class="ip-item-header">
                    <code class="ip-address">${escapeHtml(w.ip)}</code>
                    <span class="ip-badge whitelist">WHITELISTED</span>
                </div>
                <div class="ip-item-body">
                    ${w.note ? `<div><strong>Note:</strong> ${escapeHtml(w.note)}</div>` : ''}
                    <div><strong>Added by:</strong> ${escapeHtml(w.addedBy)}</div>
                    <div><strong>Since:</strong> ${new Date(w.createdAt).toLocaleString()}</div>
                    <div class="ip-item-actions">
                        <button class="admin-btn orange" data-remove-wl="${escapeHtml(w.ip)}">🗑️ Remove</button>
                    </div>
                </div>
            </div>
        `).join('');
        ...
        container.querySelectorAll('[data-remove-wl]').forEach(btn => {
            btn.addEventListener('click', () => removeIpWhitelist(btn.dataset.removeWl));
        });
    } catch (e) {
        container.innerHTML = `<div class="ip-empty">❌ ${escapeHtml(e.message)}</div>`;
    }
...
async function addIpWhitelist() {
    const ip = document.getElementById('ipWhitelistInput').value.trim();
    const note = document.getElementById('ipWhitelistNote').value.trim();
    if (!ip) return showToast('Enter an IP address');
    try {
        const res = await authFetch('/api/admin/ip-whitelist', {
            method: 'POST',
        ...
        const data = await res.json();
        if (data.success) {
            showToast(`✅ Whitelisted ${ip}`);
            document.getElementById('ipWhitelistInput').value = '';
            document.getElementById('ipWhitelistNote').value = '';
...
async function removeIpWhitelist(ip) {
    ...
    try {
        const res = await authFetch(`/api/admin/ip-whitelist/${encodeURIComponent(ip)}`, { method: 'DELETE' });
        const data = await res.json();
        if (data.success) {
...
async function loadIpLogs() {
    const search = document.getElementById('ipLogSearchInput')?.value.trim() || '';
    const container = document.getElementById('ipLogList');
    if (!container) return;
    container.innerHTML = '<div class="ip-empty">⏳ Loading...</div>';
    try {
        const params = new URLSearchParams();
        if (search) {
            if (/^[\d:.]+$/.test(search)) params.set('ip', search);
            else params.set('username', search);
        }
        params.set('limit', '200');
        const res = await authFetch(`/api/admin/ip-logs?${params}`);
        const data = await res.json();
        if (!data.success || !data.logs.length) {
            container.innerHTML = '<div class="ip-empty">No logs found</div>';
            return;
        }
        container.innerHTML = data.logs.map(l => {
            const actionClass = {
                ...
                'login_fail': 'fail',
                'banned_access': 'banned',
                'register': 'info'
            }[l.action] || 'info';
            return `
                <div class="ip-log-item ${actionClass}">
                    <div class="ip-log-time">${new Date(l.createdAt).toLocaleString()}</div>
                    <div class="ip-log-main">
                        <code>${escapeHtml(l.ip)}</code>
                        ${l.username ? `<span class="ip-log-user">👤 ${escapeHtml(l.username)}</span>` : ''}
                        <span class="ip-log-action">${escapeHtml(l.action)}</span>
                    </div>
                    ${l.userAgent ? `<div class="ip-log-ua">${escapeHtml(l.userAgent.slice(0, 80))}</div>` : ''}
                </div>
            `;
        }).join('');
    } catch (e) {
...
// ⭐ Wire up IP dashboard buttons
(function initIpDashboard() {
    const banBtn = document.getElementById('ipBanBtn');
    const wlBtn = document.getElementById('ipWhitelistBtn');
    const searchInput = document.getElementById('ipSearchInput');
    const statusFilter = document.getElementById('ipStatusFilter');
    const logSearch = document.getElementById('ipLogSearchInput');
    const logRefresh = document.getElementById('ipLogRefreshBtn');

    if (banBtn) banBtn.addEventListener('click', banIp);
    if (wlBtn) wlBtn.addEventListener('click', addIpWhitelist);
    if (searchInput) {
        let timer;
        searchInput.addEventListener('input', () => {
            clearTimeout(timer);
            timer = setTimeout(loadIpBans, 300);
        });
    }
    if (statusFilter) statusFilter.addEventListener('change', loadIpBans);
    if (logSearch) {
        let timer;
        logSearch.addEventListener('input', () => {
            clearTimeout(timer);
            timer = setTimeout(loadIpLogs, 300);
        });
    }
    if (logRefresh) logRefresh.addEventListener('click', loadIpLogs);

    document.querySelectorAll('.ip-tab').forEach(tab => {
        tab.addEventListener('click', () => switchIpTab(tab.dataset.ipTab));
    });
})();
...
async function submitSupportTicket() {
    ...
    try {
        const res = await authFetch('/api/support/submit', { method: 'POST', body: fd });
        const data = await res.json();
        if (data.success) {
            responseDiv.innerHTML = `<span style="color:green;">✅ ${t('ticket_submitted')} ID: ${escapeHtml(data.ticketId)}</span>`;
            setTimeout(() => closeSupportModal(), 2000);
        } else responseDiv.innerHTML = `<span style="color:red;">❌ ${escapeHtml(data.error)}</span>`;
...
async function showAdminSupportPanel() {
    ...
    try {
        const res = await authFetch(`/api/support/tickets?status=all`);
        ...
        let html = `<div style="margin-bottom:12px;text-align:right;"><button id="clearAllTicketsBtn" style="background:#ef4444;color:white;border:none;padding:8px 16px;border-radius:40px;font-weight:600;cursor:pointer;">🗑️ Clear All</button></div>`;

        if (!data.length) {
            container.innerHTML = html + `<div style="padding:12px;text-align:center;color:#6b7280;">${t('no_tickets')}</div>`;
            document.getElementById('clearAllTicketsBtn').addEventListener('click', clearAllSupportTickets);
            return;
        }
        ...
        html += data.map(ticket => `
            <div class="ticket-item">
                ...
                <div style="margin-top:10px;">
                    <button data-reply-ticket="${escapeHtml(ticket.ticketId)}">${t('reply_btn')}</button>
                    ${ticket.status !== 'resolved' ? `<button data-resolve-ticket="${escapeHtml(ticket.ticketId)}">${t('resolve_btn')}</button>` : ''}
                </div>
            </div>
        `).join('');
        container.innerHTML = html;
        ...
        document.getElementById('clearAllTicketsBtn').addEventListener('click', clearAllSupportTickets);
        container.querySelectorAll('[data-reply-ticket]').forEach(btn => {
            btn.addEventListener('click', () => openReplyTicketModal(btn.dataset.replyTicket));
        });
        container.querySelectorAll('[data-resolve-ticket]').forEach(btn => {
            btn.addEventListener('click', () => resolveTicket(btn.dataset.resolveTicket));
        });
    } catch (err) {
...
async function clearAllSupportTickets() {
    ...
    try {
        const res = await authFetch('/api/support/clear-all', { method: 'POST' });
        const data = await res.json();
        if (data.success) {
...
async function sendSupportReply() {
    ...
    try {
        const res = await authFetch('/api/support/reply', {
            method: 'POST',
...
async function resolveTicket(ticketId) {
    ...
    try {
        const res = await authFetch('/api/support/resolve', {
            method: 'POST',
...
function closeRoomMenuOnClickOutside(event) {
    ...
    if (menu && btn && !menu.contains(event.target) && !btn.contains(event.target)) closeRoomMenu();
}
...
function initSocket() {
    ...
    socket.on('connect_error', (err) => {
        ...
        if (err.message && err.message.includes('banned')) {
            alert('🚫 ' + (t('banned_notification') || 'Your IP has been banned. Contact support if this is a mistake.'));
            localStorage.clear();
            return;
        }
    ...
    socket.on('new-private-message', (msg) => {
        ...
        else addNotification(t('new_message'), `${msg.from}: ${String(msg.message).substring(0, 50)}`, { type: 'private_message', from: msg.from });
    });
    ...
    socket.on('new-support-ticket', (data) => {
        if (currentUser && currentUser.isAdmin) addNotification(t('new_support_ticket'), `${data.username}: ${data.subject}`);
    });
    socket.on('room-join-denied', (msg) => alert(msg));
    socket.on('error', (msg) => {
        if (msg === 'Rate limit exceeded') return;
        showToast('⚠️ ' + msg);
    });
...
async function showCoinHistoryModal() {
    ...
    try {
        const res = await authFetch(`/api/get-coin-history`);
...
async function sendManualCoinRequest() {
    ...
    const res = await authFetch('/api/manual-coin-request', { method: 'POST', body: fd });
    const data = await res.json();
...
async function startCryptoPayment(packageId) {
    try {
        ...
        const res = await authFetch('/api/nowpayments/create-invoice', {
            method: 'POST',
...
function pollCryptoPayment(orderId, fromReturn = false) {
    ...
    const tick = async () => {
        attempts++;
        try {
            const res = await authFetch(`/api/nowpayments/status/${orderId}`);
            const data = await res.json();
            if (!data.success) return;
...
const savedLangInit = localStorage.getItem('app_language');
if (savedLangInit && translations[savedLangInit]) setLanguage(savedLangInit);
...
autoLogin();
handlePaymentReturn();
console.log('⚡ Chat app loaded (SECURE + IP-BAN + IME fixed)');
