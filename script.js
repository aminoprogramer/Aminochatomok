...
// ⭐ UPDATED: Filter active tickets only (open + in-progress)
async function showAdminSupportPanel() {
    ...
    try {
        const res = await fetch(`/api/support/tickets?admin=${encodeURIComponent(currentUser.username)}&status=active`);
...
async function resolveTicket(ticketId) {
    ...
    try {
        ...
        if (data.success) { 
            ...
            delete adminTicketsCache[ticketId];
            showAdminSupportPanel(); 
        }
...
function initSocket() {
    ...
    // ⭐ UPDATED: Support reply handler
    socket.on('support-reply', (data) => { 
        playNotificationSound(); 
        addNotification(
            t('support_reply'), 
            `Admin ${t('replied_ticket')} ${data.ticketId}: ${data.reply.substring(0,50)}...`, 
            { type: 'support_ticket', ticketId: data.ticketId, reply: data.reply }
        ); 
        showToast(`📬 ${t('new_reply_support')} ${data.ticketId}`); 
    });

    // ⭐ UPDATED: Support resolved handler with auto-open
    socket.on('support-resolved', (data) => { 
        ...
        addNotification(
            ...
            { 
                ...
                reply: data.reply || 'Ticket resolved by admin.'
            }
        ); 
        showToast(`✅ ${t('ticket_resolved')} ${data.ticketId}.`);
        
        if (data.reply) {
            setTimeout(() => openViewTicketModal(data.ticketId, data.reply), 500);
        }
    });
