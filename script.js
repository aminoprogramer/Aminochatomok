...
// ⭐ applyLanguage — محسّن مع حفظ التركيز + دعم IME
function applyLanguage() {
    const isRTL = currentLanguage === 'ar';
    document.documentElement.setAttribute('dir', isRTL ? 'rtl' : 'ltr');
    document.documentElement.lang = currentLanguage;

    // ⭐ حفظ الحقل النشط لمنع فقدان التركيز
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
            // ⭐ لا تلمس placeholder إذا كان الحقل نشطاً ويحتوي نصاً
...
// ⭐ refreshUserData — with dedup guard
...
async function login() {
    ...
    if (data.success) {
        ...
        // ⭐⭐⭐ تطبيق الاتجاه فوراً قبل أي شيء
...
// ⭐⭐⭐ الحل الأساسي: handleChatKeyDown مع دعم IME الكامل ⭐⭐⭐
function handleChatKeyDown(event, target) {
    // ⭐ تجاهل Enter إذا كان المستخدم لا يزال يؤلف النص العربي (IME composition)
...
function onRoomTyping() {
    if (currentRoom && socket) {
        // ⭐ لا ترسل typing events أثناء IME composition (عربي/صيني/ياباني)
...
// ⭐⭐⭐ IME tracking — يحل مشكلة الكتابة العربية على كل الشاشات ⭐⭐⭐
function setupIMECompositionTracking() {
    ...
    inputs.forEach(({ id, flag }) => {
        ...
        // ⭐ ضبط تلقائي للاتجاه حسب النص المُدخل
        el.addEventListener('input', () => {
            if (window[flag]) return;
            autoAdjustDirection(el);
        });

        // ⭐ منع المتصفح من التدخل
...
// ==================== INIT ====================
// ⭐⭐⭐ تطبيق الاتجاه واللغة عند تحميل الصفحة ⭐⭐⭐
...
if (langSwitcher) {
    ...
    langSwitcher.addEventListener('change', (e) => {
        // ⭐ لا تغير اللغة إذا كان المستخدم يكتب بنشاط
        const activeEl = document.activeElement;
...
// ⭐⭐⭐ تهيئة IME فوراً على شاشة تسجيل الدخول والتسجيل والاسترجاع ⭐⭐⭐
setupIMECompositionTracking();

autoLogin();
handlePaymentReturn();
console.log('⚡ Chat app loaded (TURBO + IME fixed)');
