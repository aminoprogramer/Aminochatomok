...
function setSafeHTML(element, html) {
  if (element) element.innerHTML = DOMPurify.sanitize(html, {
    ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'a', 'p', 'br', 'span', 'img', 'audio', 'video', 'source'],
    ALLOWED_ATTR: ['href', 'src', 'alt', 'class', 'controls', 'preload', 'autoplay', 'muted', 'loop'],
...
// (keep your translations object unchanged)
const translations = {
  en: {
...
function showPrivateChat(username) {
  ...
  document.getElementById('giftsView').classList.remove('active');
  updatePrivateStatus();
  // Load private messages? Could be done via socket or fetch – we'll keep socket.
}
...
function openReplyTicketModal(ticketId) {
  ...
  document.getElementById('replyUserMessage').innerText = ''; // We'll fetch later if needed
...
function openViewTicketModal(ticketId) {
  ...
  // We don't have the reply yet – we could fetch, but we can just show a placeholder.
  document.getElementById('viewTicketReply').innerHTML = 'Loading...';
  // In real implementation, you'd fetch the ticket data.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Room members: show profile via avatar
  ...
  // Room settings member management: mod / remove
  ...
  // Admin pending requests: approve/reject
  ...
  // Admin support tickets: reply/resolve
  ...
  // Coin purchase buttons (card/crypto)
  ...
  // Notifications dropdown: click item
