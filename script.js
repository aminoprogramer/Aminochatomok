...
async function refreshUserData() {
    ...
    try {
        ...
        if (data.success) {
            ...
            updateProfilePic();
            updateVipBadge();
            updateGamesBalance(); // ⭐ LUCKY GAMES
        }
...
async function playGame(game, choice) {
    ...
    if (!bet || bet < gameConfig.minBet) {
        return alert(`Minimum bet is ${gameConfig.minBet} coins`);
    }
    if (bet > gameConfig.maxBet) {
        return alert(`Max bet is ${gameConfig.maxBet.toLocaleString()} coins`);
    }
        ...
        resultEl.innerHTML = `
            <div class="${winClass}">
                ...
                <div class="game-result-line">
                    Bet: ${data.bet} · Payout: ${data.payout} · ${sign}${data.profit}
                </div>
            </div>`;

        loadGameHistory();
    } catch (err) {
        resultEl.innerHTML = `<div class="game-lose">❌ Network error</div>`;
    } finally {
        document.querySelectorAll('.choice-btn, .spin-btn').forEach(b => b.disabled = false);
    }
}
...
async function loadGameHistory() {
    ...
    try {
        ...
        if (!data.success || !data.logs.length) {
            cont.innerHTML = '<div style="text-align:center;color:#999;padding:10px;">No games played yet</div>';
            return;
        }
        ...
        cont.innerHTML = data.logs.map(log => {
            ...
            return `<div class="game-history-item ${cls}">
                ...
                <span class="gh-bet">Bet: ${log.bet}</span>
...
function initSocket() {
    ...
    // ⭐ LUCKY GAMES: server-pushed result
