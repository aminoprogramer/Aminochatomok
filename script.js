...
// Emoji regex — matches any leading emoji/flag/ZWJ sequence
...
function applyLanguage() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
            ...
            if (htmlEmoji) {
                el.innerText = htmlEmoji + ' ' + cleanTranslation;
            } else {
                el.innerText = cleanTranslation || translation;
            }
        }
    });
...
async function translateText(text, targetLang) {
    ...
    // ⭐ ما تترجمش لو النص بنفس اللغة الهدف
    const detected = detectTextLanguage(text);
    if (detected === targetLang) return text;

    try {
        // ⭐ Timeout 5 ثواني عشان ما يعلقش
...
async function loadPendingNotifications() {
    ...
    try {
        ...
        console.log(`📬 Loaded ${data.notifications.length} pending notifications`);
...
function renderActiveRooms(rooms) {
    ...
    const active = (rooms || []).filter(r => r.members > 0)
...
// ==================== SEND MESSAGES (no escape — escape happens on display) ====================
...
async function translateAndDisplayMessage(msg, containerId, isSentByMe) {
    ...
    // ⭐ عرض الرسالة فوراً بدون انتظار الترجمة
    if (msg.type === 'image') {
    ...
    // ⭐ الترجمة في الخلفية بعد ما الرسالة تتعرض — ما تعلقش الواجهة
    if (msg.type !== 'image' && msg.type !== 'audio' && autoTranslateMessages && targetLanguage !== 'en') {
        ...
        // ما تترجمش لو النص أصلاً بنفس اللغة الهدف أو غير معروف
...
async function showAllUsers() { const res = await fetch(`/api/admin/get-all-users?admin=${currentUser.username}`); const users = await res.json(); if (users.error) { alert('Admin access required'); return; } const html = users.map(u => `<div>${escapeHtml(u.username)} (💰 ${u.coins}) ${u.isAdmin ? '👑 Admin' : ''} ${u.privateCode ? `🔑 <code style="font-size:11px;">${escapeHtml(u.privateCode)}</code>` : ''} <button onclick="deleteUser('${escapeHtml(u.username)}')">${t('delete')}</button></div>`).join(''); document.getElementById('allUsersList').innerHTML = html; document.getElementById('usersModal').classList.add('active'); }
...
async function showAdminSupportPanel() {
    ...
    try {
        ...
        let html = `<div style="margin-bottom:12px;text-align:right;">
            <button onclick="clearAllSupportTickets()" style="background:#ef4444;color:white;border:none;padding:8px 16px;border-radius:40px;font-weight:600;cursor:pointer;">
                🗑️ Clear All Tickets
            </button>
        </div>`;
...
async function clearAllSupportTickets() {
    if (!confirm('Are you sure you want to clear ALL support tickets? This cannot be undone.')) {
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            showToast(`✅ Cleared ${data.deletedCount} tickets (${data.notifiedOnline} online, ${data.notifiedOffline} offline)`);
            showAdminSupportPanel();
        } else {
            alert('Failed to clear tickets: ' + (data.error || 'Unknown error'));
        }
    } catch (err) {
...
async function resolveTicket(ticketId) {
    if (!confirm('Mark this ticket as resolved? The ticket will be closed and the user will be notified.')) return;
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('✅ Ticket resolved and closed');
            showAdminSupportPanel();
        }
...
function initSocket() {
    ...
    socket.on('target-updated', (data) => {
        showToast(`🎯 +${data.contribution} ${t('target')} ${t('from')} ${data.from} | ${t('total')}: ${data.monthlyTarget.toLocaleString()} ($${data.dollarsEarned})`);
...
async function showCoinHistoryModal() {
    ...
    listContainer.innerHTML = `<div style="text-align:center;padding:20px;color:#6b7280;">⏳ ${t('loading_history') || 'Loading...'}</div>`;
    modal.classList.add('active');
    ...
    try {
        ...
        if (!Array.isArray(history) || !history.length) {
            listContainer.innerHTML = `<div style="text-align:center;padding:20px;color:#6b7280;">📭 ${t('no_coin_history') || 'No coin history yet.'}</div>`;
            return;
        }
        ...
        listContainer.innerHTML = `<div style="text-align:center;padding:20px;color:#ef4444;">❌ Failed to load history: ${escapeHtml(err.message)}</div>`;
    }
}
