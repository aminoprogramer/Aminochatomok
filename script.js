...
async function deleteFriend(friend) { if (confirm(`Remove ${friend}?`)) { await fetch('/api/delete-friend', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: currentUser.username, friend }) }); loadFriends(); } }
