...
// ⭐ FIX: was throwing "Failed to load support tickets" because the .map() callback
//         parameter was named `t`, which shadowed the global translator t().
//         Now uses `ticket`, caches tickets for the reply modal, and shows inline errors.
async function showAdminSupportPanel() {
    ...
    try {
        ...
        // ⭐ callback param is `ticket`, NOT `t`
...
// ⭐ FIX: now takes only the ticketId and looks up the message from cache,
//         instead of embedding an escaped string into the inline onclick.
