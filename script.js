...
// ⭐ FIX: cache of loaded support tickets, keyed by ticketId
...
const translations = {
    en: {
        ...
        search_room: "Search room...", search_users: "Search users...", search: "Search",
        friend_requests: "Friend Requests", friends_list: "Friends", send_gift: "Send Gift",
        gift_history: "Gift History", type_message: "Type a message...", clear_all: "Clear All",
    ...
    ar: {
        ...
        search_room: "ابحث عن غرفة...", search_users: "ابحث عن مستخدمين...", search: "بحث",
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (translation) {
            if (el.tagName === 'INPUT' && el.placeholder !== undefined) {
                ...
                if (emojiMatch) el.innerText = emojiMatch[0] + ' ' + translation;
...
// ⭐ FIX: cleaner signature + `options` param so callers can force toasts
//         even when the title is translated (e.g. Arabic).
function addNotification(title, message, onClickData, options = {}) {
    ...
    if (options.silentToast === true) return;

    const importantTitles = [
        'New Message', 'Friend Request', 'Gift Received',
        'Support Reply', 'Ticket Resolved'
    ];
    const isImportant = options.important === true || importantTitles.includes(title);
    if (isImportant) showToast(`${title}: ${message}`);
}

// ⭐ NEW: on login, fetch any notifications the user missed while offline
//         and feed them through the same UI pipeline (badge + dropdown + toast).
async function loadPendingNotifications() {
    if (!currentUser || !currentUser.username) return;
    try {
        const res = await fetch(
            `/api/notifications?username=${encodeURIComponent(currentUser.username)}`
        );
        const data = await res.json();
        if (!data.success || !Array.isArray(data.notifications)) return;
        if (data.notifications.length === 0) return;

        // Newest-first from server; reverse to show oldest first so the
        // most recent ends up on top after unshift()
        const ordered = [...data.notifications].reverse();

        ordered.forEach(n => {
            if (n.type === 'support_reply') {
                const replyText = n.data?.reply || '';
                const preview = replyText.length > 50 ? replyText.substring(0, 50) + '…' : replyText;
                addNotification(
                    t('support_reply'),
                    `Admin ${t('replied_ticket')} ${n.data?.ticketId || ''}: ${preview}`,
                    { type: 'support_ticket', ticketId: n.data?.ticketId, reply: n.data?.reply },
                    { important: true }
                );
            } else if (n.type === 'support_resolved') {
                addNotification(
                    t('ticket_resolved'),
                    `${t('your_ticket')} ${n.data?.ticketId || ''} ${t('resolved')}.`,
                    null,
                    { important: true }
                );
            } else {
                addNotification(n.title || 'Notification', n.message || '', null, { important: true });
            }
        });

        // Mark them read so they don't reappear next login
        await fetch('/api/notifications/mark-read', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                username: currentUser.username,
                ids: data.notifications.map(n => n._id)
            })
        });
    } catch (err) {
...
function handleNotificationClick(idx) {
    ...
    if (!n) return;
    if (n.onClickData?.type === 'private_message' && currentPrivateChat !== n.onClickData.from) {
...
// ⭐ FIX: renamed local `t` → `toast` so it no longer shadows the global translator t()
...
async function login() {
    ...
    if (data.success) {
        ...
        loadPendingNotifications();   // ⭐ FIX: catch up on missed support notifications
        ...
        if (data.streakInfo) {
            if (data.streakInfo.payoutCreated && data.streakInfo.payoutInfo) {
                ...
                showToast(`🔥 Day ${data.streakInfo.streak}/10`);
            }
        }
...
async function sendGift() {
    ...
    const data = await res.json();
    if (data.success) {
        alert(`🎁 Gift sent! ${to} received +${data.targetContribution} target (no coins added).`);
        updateCoins();
        loadGiftHistory();
        closeSendGiftModal();
        addNotification(t('gift_received'), `You sent ${amount} coins to ${to} → +${data.targetContribution} target`);
...
async function loadTargetInfo() {
    ...
    try {
        ...
        if (reward) reward.textContent = `💰 Earned: $${data.dollarsEarned} | Total paid: $${data.totalPaid || 0}`;

        if (streakBadge) {
            streakBadge.textContent = `🔥 ${streak}/10 days`;
        ...
        if (monthCountdown && typeof data.daysUntilMonthEnd === 'number') {
            monthCountdown.textContent = `📅 ${data.daysUntilMonthEnd} day${data.daysUntilMonthEnd === 1 ? '' : 's'} until month reset`;
        }
...
async function loadPayoutHistory() {
    ...
    try {
        ...
        if (!cont) return;
        if (!payouts.length) {
            cont.innerHTML = '<div style="text-align:center;color:#999;padding:10px;">No payouts yet</div>';
        } else {
            cont.innerHTML = payouts.map(p => `
                <div class="payout-item">
                    <strong>${p.month}</strong> · ${p.targetAmount.toLocaleString()} target →
...
async function showRoomGiftModal() {
    if (!currentRoom) { showToast('Join a room first'); return; }
...
async function showAdminPayouts() {
    ...
    const html = payouts.length ? payouts.map(p => `
        <div class="payout-item">
            ...
            <b>$${p.dollarsEarned}</b>
            <span class="payout-status ${p.status}">${p.status}</span>
            ${p.status !== 'paid' ? `<button class="admin-btn green" style="margin-left:8px;" onclick="markPayoutPaid('${p._id}')">Mark Paid</button>` : ''}
        </div>
...
// ⭐ FIX: the .map() callback is now named `ticket` (not `t`), which previously
//         shadowed the global translator t() and threw "t is not a function".
//         Tickets are cached by ticketId so the reply modal never needs
//         fragile inline string escaping.
async function showAdminSupportPanel() {
    ...
    container.innerHTML = '<div style="padding:12px;text-align:center;color:#6b7280;">⏳ Loading tickets...</div>';
    ...
    try {
        ...
        if (!data.length) {
            container.innerHTML = '<div style="padding:12px;text-align:center;color:#6b7280;">No support tickets.</div>';
            return;
        }
        ...
        container.innerHTML = data.map(ticket => `
            <div class="ticket-item">
                ...
                <strong>From:</strong> ${escapeHtml(ticket.username)}<br>
                <strong>Subject:</strong> ${escapeHtml(ticket.subject)}<br>
                <strong>Message:</strong> ${escapeHtml(ticket.message)}<br>
                ${ticket.screenshot ? `<a href="${escapeHtml(ticket.screenshot)}" target="_blank" rel="noopener">📷 View Screenshot</a><br>` : ''}
                ${ticket.adminReply ? `<strong>Admin Reply:</strong> ${escapeHtml(ticket.adminReply)}<br>` : ''}
                <div style="margin-top:10px;">
                    <button onclick="openReplyTicketModal('${escapeHtml(ticket.ticketId)}')">${t('reply') || 'Reply'}</button>
                    ${ticket.status !== 'resolved'
                        ? `<button onclick="resolveTicket('${escapeHtml(ticket.ticketId)}')">${t('resolve') || 'Resolve'}</button>`
                        : ''}
                </div>
            </div>
        `).join('');
        ...
        container.innerHTML = `<div style="padding:12px;color:#ef4444;">❌ Failed to load support tickets: ${escapeHtml(err.message)}</div>`;
    }
}
...
// ⭐ FIX: takes only the ticketId and looks up the message from cache.
...
function initSocket() {
    ...
    socket.on('target-updated', (data) => {
        showToast(`🎯 +${data.contribution} target from ${data.from} | Total: ${data.monthlyTarget.toLocaleString()} ($${data.dollarsEarned})`);
    ...
    // ⭐ FIX: richer toast + persisted notifications for support events.
    socket.on('support-reply', (data) => {
        playNotificationSound();
        const preview = data.reply.length > 50 ? data.reply.substring(0, 50) + '…' : data.reply;
        addNotification(
            t('support_reply'),
            `Admin ${t('replied_ticket')} ${data.ticketId}: ${preview}`,
            { type: 'support_ticket', ticketId: data.ticketId, reply: data.reply },
            { important: true }
        );
        showToast(`📬 ${t('new_reply_support')} ${data.ticketId}`);
        // Auto-mark the live one as read so it doesn't reappear on next login
...
async function startCryptoPayment(packageId) {
    try {
        showToast('Creating crypto invoice...');
        ...
        if (!win) showToast('Please allow pop-ups to complete payment');
    ...
    const tick = async () => {
        ...
        try {
            ...
            if (data.credited || data.status === 'finished' || data.status === 'confirmed') {
                clearInterval(iv);
                showToast(`✅ Payment confirmed! +${data.coins} coins`);
                addNotification('💰 Payment Confirmed', `${data.coins} coins added (order ${orderId})`);
                await updateCoins();
                return;
            }
