...
async function login() {
    ...
    try {
        ...
        if (!data || !data.success) {
            return alert('Login failed: ' + (data?.error || 'Unknown error'));
        }
        ...
        // ⭐ Best-effort async calls (don't crash if they fail)
        try { await refreshUserData(true); } catch (e) { console.warn('refreshUserData', e); }
        try { await checkAndUpdateVIPStatus(); } catch (e) { console.warn('checkVIP', e); }

        setLanguage(currentLanguage);
        try { loadFriends(); } catch (e) { }
        try { loadFriendRequests(); } catch (e) { }
        try { loadGiftHistory(); } catch (e) { }

        if (currentUser.isAdmin) {
            const adminSection = document.getElementById('adminSection');
            const adminLookupSection = document.getElementById('adminCodeLookupSection');
            if (adminSection) adminSection.style.display = 'block';
            if (adminLookupSection) adminLookupSection.style.display = 'block';
        }
...
function showRooms() {
    ...
    if (socket) socket.emit('get-room-list');
...
async function loadGiftHistory() {
    ...
    } catch (e) { console.error('loadGiftHistory', e); }
}
...
async function acceptFriendRequest(from) {
    if (socket) socket.emit('accept-friend-request-socket', { from });
}
...
async function sendFriendRequest(to) {
    if (socket) socket.emit('send-friend-request-socket', { to });
}
...
async function updateCoins() {
    ...
    } catch (e) { console.error('updateCoins', e); }
}
...
function boostRoom() {
    ...
    if (amount && currentRoom && currentUser.coins >= amount) {
        if (socket) socket.emit('boost-room', { roomName: currentRoom, amount });
        closeBoostModal();
        updateCoins();
    } else {
...
async function loadRoomMembers() {
    ...
    if (data.success) {
        ...
        if (kickedCont && (currentUser.username === currentRoomOwner || currentRoomModerators.includes(currentUser.username)) && data.kicked && data.kicked.length) {
        ...
        } else if (kickedCont) kickedCont.innerHTML = '';
    }
}
...
function addRoomMember() {
    ...
    if (!username) return;
    if (socket) socket.emit('add-room-member', { roomName: currentRoom, username });
    document.getElementById('addMemberUsername').value = '';
}
function removeRoomMember(username) {
    if (!confirm(`Remove ${username}?`)) return;
    if (socket) socket.emit('remove-room-member', { roomName: currentRoom, username });
}
function setModerator(username) { if (socket) socket.emit('set-moderator', { roomName: currentRoom, username }); }
function removeModerator(username) { if (socket) socket.emit('remove-moderator', { roomName: currentRoom, username }); }
...
function endCall() {
    ...
    if (callActive && currentPrivateChat && socket) socket.emit('end-call', { to: currentPrivateChat });
    callActive = false;
    pendingCallFrom = null;
    pendingOffer = null;
...
function updateYouTubeVideo(videoId) {
    ...
    if (currentRoom && socket) socket.emit('sync-youtube', { room: currentRoom, videoId });
...
function initSocket() {
    ...
    socket = io(socketURL, {
        ...
        timeout: 8000,
        transports: ['polling', 'websocket'],  // ⭐ FIXED: polling first, then upgrade
    ...
    // ⭐ FIXED: Socket error handler (prevents silent rate-limit failures)
    socket.on('error', (err) => {
        const msg = err?.message || err || '';
        if (msg === 'Rate limit exceeded') return;
        console.error('Socket error:', msg);
        if (typeof msg === 'string' && msg !== 'Rate limit exceeded') {
            showToast('⚠️ ' + msg);
        }
    });
...
async function autoLogin() {
    ...
    try {
        ...
        // ⭐ Init socket AFTER screen switch
        initSocket();

        // ⭐ Best-effort async calls
        try { await refreshUserData(true); } catch (e) { console.warn('refreshUserData', e); }
        try { await checkAndUpdateVIPStatus(); } catch (e) { console.warn('checkVIP', e); }
...
// ⭐ FIX: Null-safe target language listener
const targetLangEl = document.getElementById('targetLanguage');
if (targetLangEl) {
    targetLangEl.value = targetLanguage;
    targetLangEl.addEventListener('change', function (e) {
        targetLanguage = e.target.value;
        localStorage.setItem('targetLanguage', targetLanguage);
...
const autoTranslateEl = document.getElementById('autoTranslateMessages');
if (autoTranslateEl) {
    autoTranslateEl.checked = autoTranslateMessages;
    autoTranslateEl.addEventListener('change', function (e) {
...
// ⭐⭐⭐ CRITICAL FIX: Force login screen visible on load
document.getElementById('loginScreen').style.display = 'flex';
document.getElementById('mainContainer').style.display = 'none';

// ⭐⭐⭐ FIX: autoLogin now handles errors gracefully
(async () => {
    try {
        await autoLogin();
    } catch (err) {
...
handlePaymentReturn();
console.log('⚡ Chat app loaded (SECURE + IME fixed + polling/websocket fallback)');
