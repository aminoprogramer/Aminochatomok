...
async function showAdminSupportPanel() {
    ...
    } catch (err) { alert('Failed to load support tickets.'); }
}
...
async function purchaseCoins(coins, name, price, method) {
    ...
    if (method === 'nowpayment') {
        ...
        // Card flow still uses the manual request path (no card gateway configured)
...
function pollCryptoPayment(orderId, fromReturn = false) {
    if (!orderId) return;
    let attempts = 0;
    const MAX_ATTEMPTS = 120; // ~20 min at 10s intervals
    ...
    // Check immediately if we came back from the invoice page
    if (fromReturn) tick();
    const iv = setInterval(tick, 10000);
}

// Handle redirect back from NOWPayments (?payment=success&order=MK-...)
