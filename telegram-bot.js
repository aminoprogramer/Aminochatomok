...
//
//      # If WEBHOOK_URL is not set, falls back to polling (local dev).
// ============================================================
...
function startTelegramBot(ctx) {
    ...
    if (useWebhook) {
        ...
        // Expose the webhook endpoint on the existing Express app
        if (app) {
            ...
            // Clear any old webhook/pending updates, then register the new one
            ...
            bot = new TelegramBot(token, { polling: { interval: 2000, autoStart: true, params: { timeout: 30 } } });
        }
    } else {
