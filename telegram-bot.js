...
function startTelegramBot(ctx) {
    ...
    const { io, onlineUsers, models, helpers } = ctx;
    const {
    ...
    bot.onText(/^\/broadcast\s+([\s\S]+)$/, guard(async (msg, match) => {
        ...
        // persist as notification for every user
    ...
    bot.on('callback_query', async (q) => {
        ...
        try {
            ...
            if (ns === 'users' && action === 'page') {
                // re-use the /users handler by faking a match
            ...
            if (ns === 'r' && action === 'confirmdelete') {
                ...
                io.emit('room-list', await Room.find({}, 'name members boostLevel isVipRoom membersOnly').then(rs =>
                    rs.map(x => ({ name: x.name, members: x.members.length, boostLevel: x.boostLevel, isVipRoom: x.isVipRoom, membersOnly: x.membersOnly }))
                ));
    ...
    bot.on('webhook_error', (err) => console.error('TG webhook error:', err.message));
